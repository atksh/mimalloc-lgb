...
#define R_API_END()                         \
  ...
  {                                         \
    R_INT_PTR(call_state)                   \
    [0] = -1;                               \
    LGBM_SetLastError("unknown exception"); \
    return call_state;                      \
  }                                         \
  return call_state;
...
LGBM_SE EncodeChar(LGBM_SE dest, const char *src, LGBM_SE buf_len, LGBM_SE actual_len, size_t str_len)
{
  ...
  R_INT_PTR(actual_len)
  [0] = static_cast<int>(str_len);
                                ...
                                LGBM_SE call_state)
{
  ...
  std::vector<int, mi_stl_allocator<int>> idxvec(len);
                                      ...
                                      LGBM_SE call_state)
{
  ...
  std::vector<const char *, mi_stl_allocator<const char *>> vec_sptr;
                                      ...
                                      LGBM_SE call_state)
{
  ...
  std::vector<std::vector<char, mi_stl_allocator<char>>> names(len);
  std::vector<char *, mi_stl_allocator<char *>> ptr_names(len);
  for (int i = 0; i < len; ++i)
  {
                               ...
                               LGBM_SE call_state)
{
  ...
  {
    std::vector<int32_t, mi_stl_allocator<int32_t>> vec(len);
#pragma omp parallel for schedule(static, 512) if (len >= 1024)
    for (int i = 0; i < len; ++i)
    {
  ...
  {
    std::vector<float, mi_stl_allocator<float>> vec(len);
#pragma omp parallel for schedule(static, 512) if (len >= 1024)
    for (int i = 0; i < len; ++i)
    {
...
#pragma omp parallel for schedule(static, 512) if (out_len >= 1024)
    ...
    {
      R_INT_PTR(field_data)
      [i] = p_data[i + 1] - p_data[i];
    }
  }
  else if (!strcmp("init_score", name))
  {
                                          ...
                                          LGBM_SE call_state)
{
  ...
  std::vector<float, mi_stl_allocator<float>> tgrad(int_len), thess(int_len);
                                          ...
                                          LGBM_SE call_state)
{
  ...
  R_INT_PTR(out)
  [0] = static_cast<int>(out_iteration);
  R_API_END();
}
                                        ...
                                        LGBM_SE call_state)
{
  R_API_BEGIN();
  int64_t out_len = 0;
  std::vector<char, mi_stl_allocator<char>> inner_char_buf(R_AS_INT(buffer_len));
