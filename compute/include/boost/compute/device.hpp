...
namespace boost
{
    namespace compute
    {
        ...
        public:
            ...
            std::vector<std::string, mi_stl_allocator<std::string>> extensions() const
            {
                std::string extensions_string =
                    get_info<std::string>(CL_DEVICE_EXTENSIONS);
                std::vector<std::string, mi_stl_allocator<std::string>> extensions_vector;
            ...
            {
                const std::vector<std::string, mi_stl_allocator<std::string>> extensions = this->extensions();
            ...
            std::vector<device, mi_stl_allocator<device>>
            partition(const cl_device_partition_property *properties) const
            {
                ...
                // get sub-device ids
                std::vector<cl_device_id, mi_stl_allocator<cl_device_id>> ids(count);
                ...
                std::vector<device, mi_stl_allocator<device>> devices(count);
            ...
            /// \opencl_version_warning{1,2}
            std::vector<device, mi_stl_allocator<device>> partition_equally(size_t count) const
            {
            ...
            /// \opencl_version_warning{1,2}
            std::vector<device, mi_stl_allocator<device>>
            partition_by_counts(const std::vector<size_t, mi_stl_allocator<size_t>> &counts) const
            {
                std::vector<cl_device_partition_property, mi_stl_allocator<cl_device_partition_property>> properties;
