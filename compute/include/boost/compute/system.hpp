...
namespace boost
{
    namespace compute
    {
        ...
        public:
            ...
            {
                const std::vector<device, mi_stl_allocator<device>> devices = system::devices();
            ...
            /// }
            /// \endcode
            static std::vector<device, mi_stl_allocator<device>> devices()
            {
                std::vector<device, mi_stl_allocator<device>> devices;

                const std::vector<platform, mi_stl_allocator<platform>> platforms = system::platforms();
                for (size_t i = 0; i < platforms.size(); i++)
                {
                    const std::vector<device, mi_stl_allocator<device>> platform_devices = platforms[i].devices();
            ...
            /// }
            /// \endcode
            static std::vector<platform, mi_stl_allocator<platform>> platforms()
            {
                cl_uint count = 0;
                clGetPlatformIDs(0, 0, &count);

                std::vector<platform, mi_stl_allocator<platform>> platforms;
                if (count > 0)
                {
                    std::vector<cl_platform_id, mi_stl_allocator<cl_platform_id>> platform_ids(count);
        ...
        private:
            ...
            static device find_default_device()
            {
                ...
                const std::vector<device, mi_stl_allocator<device>> devices_ = devices();
                if (devices_.empty())
                {
                    BOOST_THROW_EXCEPTION(no_device_found());
                }
