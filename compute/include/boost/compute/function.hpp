...
namespace boost
{
    namespace compute
    {
        ...
        public:
            ...
            {
                return (m_name == other.m_name) && (m_definitions == other.m_definitions) && (m_source == other.m_source);
            }
        ...
        namespace detail
        {
            ...
            inline std::vector<std::string, mi_stl_allocator<std::string>> parse_argument_names(const char *arguments)
            {
                BOOST_ASSERT_MSG(
                    arguments[0] == '(' && arguments[std::strlen(arguments) - 1] == ')',
                    "Arguments should start and end with parentheses");

                std::vector<std::string, mi_stl_allocator<std::string>> args;
            ...
            struct signature_argument_inserter
            {
                ...
                size_t m_last;
                std::stringstream &s;
                std::vector<std::string, mi_stl_allocator<std::string>> m_argument_names;
            };
            ...
            inline std::string make_function_declaration(const char *name, const char *arguments)
            {
                ...
                if (arity_type::value > 0)
                {
                    ...
                    mpl::for_each<
                        typename mpl::transform<parameter_types, boost::add_pointer<mpl::_1>>::type>(i);
                }
