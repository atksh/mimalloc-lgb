...
namespace boost
{
    namespace compute
    {
        ...
        public:
            ...
            template <class T>
            boost::optional<T> get_sub_group_info(const device &device, cl_kernel_sub_group_info info,
                                                  const std::vector<size_t, mi_stl_allocator<size_t>> input) const
            {
        ...
        BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(kernel,
                                                             ((std::string, CL_KERNEL_FUNCTION_NAME))((cl_uint, CL_KERNEL_NUM_ARGS))((cl_uint, CL_KERNEL_REFERENCE_COUNT))((cl_context, CL_KERNEL_CONTEXT))((cl_program, CL_KERNEL_PROGRAM)))
...
#define BOOST_COMPUTE_DETAIL_DEFINE_KERNEL_GET_ARG_INFO_SPECIALIZATION(result_type, value) \
    ...
    {                                                                                      \
        template <>                                                                        \
        struct get_object_info_type<kernel, value>                                         \
        {                                                                                  \
            typedef result_type type;                                                      \
        };                                                                                 \
    }                                                                                      \
    template <>                                                                            \
    inline result_type kernel::get_arg_info<value>(size_t index) const                     \
    {                                                                                      \
