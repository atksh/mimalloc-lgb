...
namespace boost
{
    namespace compute
    {
        ...
        public:
            ...
            std::vector<unsigned char, mi_stl_allocator<unsigned char>> binary() const
            {
                size_t binary_size = get_info<size_t>(CL_PROGRAM_BINARY_SIZES);
                std::vector<unsigned char, mi_stl_allocator<unsigned char>> binary(binary_size);
            ...
            std::vector<unsigned char, mi_stl_allocator<unsigned char>> il_binary() const
            {
                return get_info<std::vector<unsigned char, mi_stl_allocator<unsigned char>>>(CL_PROGRAM_IL);
            }
#endif // BOOST_COMPUTE_CL_VERSION_2_1

            std::vector<device, mi_stl_allocator<device>> get_devices() const
            {
                std::vector<cl_device_id, mi_stl_allocator<cl_device_id>> device_ids =
                    get_info<std::vector<cl_device_id, mi_stl_allocator<cl_device_id>>>(CL_PROGRAM_DEVICES);

                std::vector<device, mi_stl_allocator<device>> devices;
            ...
            void compile(const std::string &options = std::string(),
                         const std::vector<std::pair<std::string, program>, mi_stl_allocator<std::pair<std::string, program>>> &headers =
                             std::vector<std::pair<std::string, program>, mi_stl_allocator<std::pair<std::string, program>>>())
            {
                const char *options_string = 0;
                ...
                {
                    std::vector<const char *, mi_stl_allocator<const char *>> header_names(headers.size());
                    std::vector<cl_program, mi_stl_allocator<cl_program>> header_programs(headers.size());
            ...
            static program link(const std::vector<program, mi_stl_allocator<program>> &programs,
            ...
            ///
            /// \see_opencl_ref{clCreateProgramWithSource}
            static program create_with_source(const std::vector<std::string, mi_stl_allocator<std::string>> &sources,
                                              const context &context)
            {
                std::vector<const char *, mi_stl_allocator<const char *>> source_strings(sources.size());
            ...
            ///
            /// \see_opencl_ref{clCreateProgramWithSource}
            static program create_with_source_file(const std::vector<std::string, mi_stl_allocator<std::string>> &files,
                                                   const context &context)
            {
                std::vector<std::string, mi_stl_allocator<std::string>> sources(files.size());
            ...
            ///
            /// \see_opencl_ref{clCreateProgramWithBinary}
            static program create_with_binary(const std::vector<unsigned char, mi_stl_allocator<unsigned char>> &binary,
                                              const context &context)
            {
                                                   ...
                                                   const context &context)
            {
                ...
                // read binary
                std::vector<unsigned char, mi_stl_allocator<unsigned char>> binary(
                    (std::istreambuf_iterator<char>(stream)),
                    std::istreambuf_iterator<char>());
            ...
            static program create_with_builtin_kernels(const context &context,
                                                       const std::vector<device, mi_stl_allocator<device>> &devices,
            ...
            static program create_with_il(const std::vector<unsigned char, mi_stl_allocator<unsigned char>> &il_binary,
                                          const context &context)
            {
                                               ...
                                               const context &context)
            {
                ...
                // read binary
                std::vector<unsigned char, mi_stl_allocator<unsigned char>> il(
                    (std::istreambuf_iterator<char>(stream)),
                    std::istreambuf_iterator<char>());
     ...
     */
        ...
        private:
            ...
            static void save_program_binary(const std::string &hash, const program &prog)
            {
                ...
                std::vector<unsigned char, mi_stl_allocator<unsigned char>> binary = prog.binary();
        ...
        BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(program,
                                                             ((cl_uint, CL_PROGRAM_REFERENCE_COUNT))((cl_context, CL_PROGRAM_CONTEXT))((cl_uint, CL_PROGRAM_NUM_DEVICES))((std::vector<cl_device_id>, CL_PROGRAM_BINARIES)))
