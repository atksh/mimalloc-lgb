...
namespace boost
{
    namespace compute
    {
        ...
        public:
            ...
            std::vector<device, mi_stl_allocator<device>> devices(cl_device_type type = CL_DEVICE_TYPE_ALL) const
            {
                size_t count = device_count(type);
                if (count == 0)
                {
                    // no devices for this platform
                    return std::vector<device, mi_stl_allocator<device>>();
                }

                std::vector<cl_device_id, mi_stl_allocator<cl_device_id>> device_ids(count);
        ...
        BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(platform,
                                                             ((std::string, CL_PLATFORM_PROFILE))((std::string, CL_PLATFORM_VERSION))((std::string, CL_PLATFORM_NAME))((std::string, CL_PLATFORM_VENDOR))((std::string, CL_PLATFORM_EXTENSIONS)))
