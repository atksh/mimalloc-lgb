...
namespace boost
{
    namespace compute
    {
        ...
        public:
            ...
            ///
            /// \see_opencl_ref{clCreateContext}
            explicit context(const std::vector<device, mi_stl_allocator<device>> &devices,
                             const cl_context_properties *properties = 0)
            {
            ...
            {
                std::vector<device, mi_stl_allocator<device>> devices = get_devices();
            ...
            {
                return get_info<std::vector<device, mi_stl_allocator<device>>>(CL_CONTEXT_DEVICES);
            }
        ...
        BOOST_COMPUTE_DETAIL_DEFINE_GET_INFO_SPECIALIZATIONS(context,
                                                             ((cl_uint, CL_CONTEXT_REFERENCE_COUNT))((std::vector<cl_device_id>, CL_CONTEXT_DEVICES))((std::vector<cl_context_properties>, CL_CONTEXT_PROPERTIES)))
