...
namespace boost
{
    namespace compute
    {
        namespace detail
        {
            ...
            // specialization for std::vector<T, mi_stl_allocator<T>>
            template <class T>
            struct get_object_info_impl<std::vector<T, mi_stl_allocator<T>>>
            {
                template <class Function, class Info>
                std::vector<T, mi_stl_allocator<T>> operator()(Function function, Info info) const
                {
                    size_t size = 0;
                    ...
                    std::vector<T, mi_stl_allocator<T>> vector(size / sizeof(T));
                ...
                template <class Function, class Info>
                std::vector<T, mi_stl_allocator<T>> operator()(Function function, Info info,
                                                               const size_t input_size, const void *input) const
                {
                    ...
                    {
                        std::vector<T, mi_stl_allocator<T>> vector(3);
                        cl_int ret = function(
