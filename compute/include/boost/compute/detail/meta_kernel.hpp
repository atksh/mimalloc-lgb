...
#define BOOST_COMPUTE_META_KERNEL_DECLARE_TYPE_STREAM_OPERATORS(type)                                     \
    ...
    BOOST_COMPUTE_META_KERNEL_DECLARE_VECTOR_TYPE_STREAM_OPERATOR(BOOST_PP_CAT(BOOST_PP_CAT(type, 16), _))
            ...
            public:
                ...
                {
                    BOOST_ASSERT(
                        (address_space == memory_object::global_memory) || (address_space == memory_object::constant_memory));
...
#undef BOOST_COMPUTE_META_KERNEL_INSERT_FUNCTION_ARGS
            ...
            private:
                ...
                std::vector<std::string, mi_stl_allocator<std::string>> m_args;
                std::string m_pragmas;
                std::string m_options;
                std::vector<detail::meta_kernel_stored_arg, mi_stl_allocator<detail::meta_kernel_stored_arg>> m_stored_args;
                std::vector<detail::meta_kernel_buffer_info, mi_stl_allocator<detail::meta_kernel_buffer_info>> m_stored_buffers;
                std::vector<detail::meta_kernel_svm_info, mi_stl_allocator<detail::meta_kernel_svm_info>> m_stored_svm_ptrs;
            };
                                           ...
                                           const detail::device_ptr_index_expr<T, IndexExpr> &expr)
            {
                if (expr.m_index == 0)
                {
                    return kernel << kernel.get_buffer_identifier<T>(expr.m_buffer) << '[' << expr.m_expr << ']';
                }
                else
                {
                    return kernel << kernel.get_buffer_identifier<T>(expr.m_buffer) << '[' << expr.m_index << "+(" << expr.m_expr << ")]";
                }
            }
...
// SVM requires OpenCL 2.0
            ...
            {
                return kernel << kernel.get_svm_identifier<T>(expr.m_svm_ptr) << '[' << expr.m_expr << ']';
            }
#endif
