...
BOOST_AUTO_TEST_CASE(sort_host_vector)
{
    ...
    std::vector<int, mi_stl_allocator<int>> vector(data, data + 8);
    boost::compute::sort(vector.begin(), vector.end(), queue);
...
BOOST_AUTO_TEST_CASE(sort_custom_struct)
{
    ...
    std::vector<Particle, mi_stl_allocator<Particle>> particles;
...
BOOST_AUTO_TEST_CASE(sort_int2)
{
    ...
    std::vector<int2_, mi_stl_allocator<int2_>> host(size, int2_(0, 0));
