...
BOOST_AUTO_TEST_CASE(get_max_work_item_sizes)
{
    ...
    std::vector<size_t, mi_stl_allocator<size_t>> max_work_item_sizes =
        device.get_info<std::vector<size_t, mi_stl_allocator<size_t>>>(CL_DEVICE_MAX_WORK_ITEM_SIZES);
...
{
    const std::vector<cl_device_partition_property, mi_stl_allocator<cl_device_partition_property>> properties =
        device.get_info<std::vector<cl_device_partition_property, mi_stl_allocator<cl_device_partition_property>>>(
...
BOOST_AUTO_TEST_CASE(partition_device_equally)
{
    ...
    std::vector<boost::compute::device, mi_stl_allocator<boost::compute::device>>
...
BOOST_AUTO_TEST_CASE(partition_by_counts)
{
    ...
    std::vector<size_t, mi_stl_allocator<size_t>> counts;
    counts.push_back(2);
    counts.push_back(1);
    counts.push_back(1);
...
BOOST_AUTO_TEST_CASE(partition_by_affinity_domain)
{
    ...
    std::vector<boost::compute::device, mi_stl_allocator<boost::compute::device>> sub_devices =
