...
{
    int n = 1024;
    std::vector<int, mi_stl_allocator<int>> host_keys(n);
    std::vector<float, mi_stl_allocator<float>> host_values(n);
    for (int i = 0; i < n; i++)
    {
        host_keys[i] = n - i;
...
BOOST_AUTO_TEST_CASE(sort_int_and_float_custom_comparison_func)
{
    ...
    int n = 1024;
    std::vector<int_, mi_stl_allocator<int_>> host_keys(n);
    std::vector<float_, mi_stl_allocator<float_>> host_values(n);
    for (int i = 0; i < n; i++)
    {
        host_keys[i] = n - i;
        host_values[i] = (n - i) / 2.f;
...
BOOST_AUTO_TEST_CASE(sort_int_and_float2)
{
    ...
    int n = 1024;
    std::vector<int_, mi_stl_allocator<int_>> host_keys(n);
    std::vector<float2_, mi_stl_allocator<float2_>> host_values(n);
    for (int i = 0; i < n; i++)
    {
        host_keys[i] = n - i;
...
BOOST_AUTO_TEST_CASE(sort_custom_struct_by_int)
{
    ...
    int_ n = 1024;
    std::vector<int_, mi_stl_allocator<int_>> host_keys(n);
    std::vector<custom_struct, mi_stl_allocator<custom_struct>> host_values(n);
    for (int_ i = 0; i < n; i++)
    {
        host_keys[i] = n - i;
        host_values[i].x = n - i;
