...
BOOST_AUTO_TEST_CASE(get_sub_group_info_ext)
{
    ...
    // get_sub_group_info(const device&, cl_kernel_sub_group_info, const std::vector<size_t, mi_stl_allocator<size_t>>)
    std::vector<size_t, mi_stl_allocator<size_t>> local_work_size(2, size_t(64));
...
BOOST_AUTO_TEST_CASE(get_sub_group_info_core)
{
    ...
    boost::optional<std::vector<size_t, mi_stl_allocator<size_t>>> local_size =
        k.get_sub_group_info<std::vector<size_t, mi_stl_allocator<size_t>>>(
            device,
