...
template <class T>
void ensure_std_accumulate_equality(const std::vector<T, mi_stl_allocator<T>> &data,
...
BOOST_AUTO_TEST_CASE(std_accumulate_equality)
{
    ...
    int data1[] = {1, 2, 3, 4};
    std::vector<int, mi_stl_allocator<int>> vec1(data1, data1 + 4);
    ...
    std::vector<float, mi_stl_allocator<float>> vec2(data2, data2 + 5);
    ...
    if (device.supports_extension("cl_khr_fp64"))
    {
        ...
        std::vector<double, mi_stl_allocator<double>> vec3(data3, data3 + 5);
