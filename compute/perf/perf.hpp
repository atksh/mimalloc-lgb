...
template <class T>
std::vector<T, mi_stl_allocator<T>> generate_random_vector(const size_t size)
{
    std::vector<T, mi_stl_allocator<T>> vector(size);
...
public:
    ...
    std::vector<boost::timer::nanosecond_type, mi_stl_allocator<boost::timer::nanosecond_type>> times;
};
