...
template <class T>
double perf_sort(const std::vector<T, mi_stl_allocator<T>> &data,
                 const size_t trials,
...
template <class T>
void tune_sort(const std::vector<T, mi_stl_allocator<T>> &data,
               const size_t trials,
               compute::command_queue &queue)
{
...
int main(int argc, char *argv[])
{
    ...
    options.add_options()("help", "show usage instructions")("size", po::value<size_t>()->default_value(8192), "input size")("trials", po::value<size_t>()->default_value(3), "number of trials to run")("tune", "run tuning procedure");
    ...
    std::vector<unsigned int, mi_stl_allocator<unsigned int>> data(size);
