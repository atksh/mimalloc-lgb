...
bool check_transposition(const std::vector<float, mi_stl_allocator<float>> &expectedResult,
                         uint_ size,
                         const std::vector<float, mi_stl_allocator<float>> &transposedMatrix)
{
...
void generate_matrix(std::vector<float, mi_stl_allocator<float>> &in, std::vector<float, mi_stl_allocator<float>> &out, uint_ rows, uint_ cols)
{
    // generate a matrix
...
int main(int argc, char *argv[])
{
    ...
    options.add_options()("help", "show usage instructions")("rows", po::value<uint_>()->default_value(4096), "number of matrix rows")("cols", po::value<uint_>()->default_value(4096), "number of matrix columns");
...
// On OSX this example does not work on CPU devices
    ...
    std::vector<float, mi_stl_allocator<float>> h_input(size);
    std::vector<float, mi_stl_allocator<float>> h_output(size);
    std::vector<float, mi_stl_allocator<float>> expectedResult(size);
