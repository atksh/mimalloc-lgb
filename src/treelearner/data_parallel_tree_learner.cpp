...
namespace LightGBM
{
  ...
  void DataParallelTreeLearner<TREELEARNER_T>::BeforeTrain()
  {
    ...
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> feature_distribution(num_machines_, std::vector<int, mi_stl_allocator<int>>());
    std::vector<int, mi_stl_allocator<int>> num_bins_distributed(num_machines_, 0);
...
#pragma omp parallel for schedule(static)
  ...
  template <typename TREELEARNER_T>
  void DataParallelTreeLearner<TREELEARNER_T>::FindBestSplitsFromHistograms(const std::vector<int8_t, mi_stl_allocator<int8_t>> &, bool, const Tree *tree)
  {
