...
#pragma omp parallel for schedule(static)
  ...
  void GPUTreeLearner::AllocateGPUMemory()
  {
    ...
    std::vector<int, mi_stl_allocator<int>> dense_dword_ind(dword_features_);
    for (int i = 0; i < num_feature_groups_; ++i)
    {
...
// device_bin_mults_.push_back(1);
    ...
    std::vector<Feature4 *, mi_stl_allocator<Feature4 *>> host4_vecs(nthreads);
    std::vector<boost::compute::buffer, mi_stl_allocator<boost::compute::buffer>> host4_bufs(nthreads);
    std::vector<Feature4 *, mi_stl_allocator<Feature4 *>> host4_ptrs(nthreads);
...
#pragma omp parallel for schedule(static)
    for (int i = 0; i < static_cast<int>(dense_feature_group_map_.size() / dword_features_); ++i)
    {
      ...
      if (dword_features_ == 8)
      {
        ...
        {
          host4[j].s[0] = (uint8_t)((iters[0].RawGet(j) * dev_bin_mult[0] + ((j + 0) & (dev_bin_mult[0] - 1))) | ((iters[1].RawGet(j) * dev_bin_mult[1] + ((j + 1) & (dev_bin_mult[1] - 1))) << 4));
          host4[j].s[1] = (uint8_t)((iters[2].RawGet(j) * dev_bin_mult[2] + ((j + 2) & (dev_bin_mult[2] - 1))) | ((iters[3].RawGet(j) * dev_bin_mult[3] + ((j + 3) & (dev_bin_mult[3] - 1))) << 4));
          host4[j].s[2] = (uint8_t)((iters[4].RawGet(j) * dev_bin_mult[4] + ((j + 4) & (dev_bin_mult[4] - 1))) | ((iters[5].RawGet(j) * dev_bin_mult[5] + ((j + 5) & (dev_bin_mult[5] - 1))) << 4));
          host4[j].s[3] = (uint8_t)((iters[6].RawGet(j) * dev_bin_mult[6] + ((j + 6) & (dev_bin_mult[6] - 1))) | ((iters[7].RawGet(j) * dev_bin_mult[7] + ((j + 7) & (dev_bin_mult[7] - 1))) << 4));
        }
      }
...
#pragma omp parallel for schedule(static)
            for (int j = 0; j < num_data_; ++j)
            {
              host4[j].s[i >> 1] |= (uint8_t)((iter.RawGet(j) * device_bin_mults_[copied_feature4 * dword_features_ + i] + ((j + i) & (device_bin_mults_[copied_feature4 * dword_features_ + i] - 1)))
                                              << ((i & 1) << 2));
            }
          }
          else
...
#pragma omp parallel for schedule(static)
            for (int j = 0; j < num_data_; ++j)
            {
              host4[j].s[i] = (uint8_t)(iter.RawGet(j) * device_bin_mults_[copied_feature4 * dword_features_ + i] + ((j + i) & (device_bin_mults_[copied_feature4 * dword_features_ + i] - 1)));
            }
          }
          else if (dynamic_cast<DenseBinIterator<uint8_t, true> *>(bin_iter) != 0)
          {
...
#pragma omp critical
  ...
  void GPUTreeLearner::InitGPU(int platform_id, int device_id)
  {
    ...
    {
      const std::vector<boost::compute::platform, mi_stl_allocator<boost::compute::platform>> platforms = boost::compute::system::platforms();
      if (static_cast<int>(platforms.size()) > platform_id)
      {
        const std::vector<boost::compute::device, mi_stl_allocator<boost::compute::device>> platform_devices = platforms[platform_id].devices();
...
#pragma omp parallel for schedule(static)
    ...
    std::vector<int8_t, mi_stl_allocator<int8_t>> is_feature_group_used(num_feature_groups_, 0);
...
#pragma omp parallel for schedule(static, 1024) reduction(+ \
                                                          : used_dense_feature_groups) if (num_dense_feature_groups_ >= 2048)
    for (int i = 0; i < num_dense_feature_groups_; ++i)
    {
  ...
  void GPUTreeLearner::ConstructHistograms(const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used, bool use_subtract)
  {
    std::vector<int8_t, mi_stl_allocator<int8_t>> is_sparse_feature_used(num_features_, 0);
    std::vector<int8_t, mi_stl_allocator<int8_t>> is_dense_feature_used(num_features_, 0);
#pragma omp parallel for schedule(static)
...
// #define GPU_DEBUG_COMPARE
    ...
    for (int i = 0; i < num_dense_feature_groups_; ++i)
    {
      ...
      if (num_data != num_data_)
      {
        train_data_->FeatureGroupBin(dense_feature_group_index)->ConstructHistogram(smaller_leaf_splits_->data_indices(), 0, num_data, ordered_gradients_.data(), ordered_hessians_.data(), current_histogram);
      }
      else
      {
        train_data_->FeatureGroupBin(dense_feature_group_index)->ConstructHistogram(0, num_data, gradients_, hessians_, current_histogram);
      }
