...
namespace LightGBM
{
  ...
  void VotingParallelTreeLearner<TREELEARNER_T>::Init(const Dataset *train_data, bool is_constant_hessian)
  {
    ...
    std::vector<int, mi_stl_allocator<int>> offsets;
  ...
  template <typename TREELEARNER_T>
  void VotingParallelTreeLearner<TREELEARNER_T>::GlobalVoting(int leaf_idx, const std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> &splits, std::vector<int, mi_stl_allocator<int>> *out)
  {
    out->clear();
    if (leaf_idx < 0)
    {
    ...
    std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> feature_best_split(this->train_data_->num_total_features(), LightSplitInfo());
    ...
    // get top k
    std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> top_k_splits;
  ...
  template <typename TREELEARNER_T>
  void VotingParallelTreeLearner<TREELEARNER_T>::CopyLocalHistogram(const std::vector<int, mi_stl_allocator<int>> &smaller_top_features, const std::vector<int, mi_stl_allocator<int>> &larger_top_features)
  {
    for (int i = 0; i < this->num_features_; ++i)
    {
  ...
  void VotingParallelTreeLearner<TREELEARNER_T>::FindBestSplits(const Tree *tree)
  {
    ...
    std::vector<int8_t, mi_stl_allocator<int8_t>> is_feature_used(this->num_features_, 0);
#pragma omp parallel for schedule(static)
    for (int feature_index = 0; feature_index < this->num_features_; ++feature_index)
    {
      if (!this->col_sampler_.is_feature_used_bytree()[feature_index])
        continue;
      if (this->parent_leaf_histogram_array_ != nullptr && !this->parent_leaf_histogram_array_[feature_index].is_splittable())
      {
    ...
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> smaller_bestsplit_per_features(this->num_features_);
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> larger_bestsplit_per_features(this->num_features_);
...
#pragma omp parallel for schedule(static)
    ...
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> smaller_top_k_splits, larger_top_k_splits;
    // local voting
    ...
    std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> smaller_top_k_light_splits(top_k_);
    std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> larger_top_k_light_splits(top_k_);
    ...
    std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> smaller_top_k_splits_global;
    std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> larger_top_k_splits_global;
    offset = 0;
    for (int i = 0; i < num_machines_; ++i)
    {
    ...
    // global voting
    std::vector<int, mi_stl_allocator<int>> smaller_top_features, larger_top_features;
  ...
  template <typename TREELEARNER_T>
  void VotingParallelTreeLearner<TREELEARNER_T>::FindBestSplitsFromHistograms(const std::vector<int8_t, mi_stl_allocator<int8_t>> &, bool, const Tree *tree)
  {
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> smaller_bests_per_thread(this->share_state_->num_threads);
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> larger_bests_per_thread(this->share_state_->num_threads);
    std::vector<int8_t, mi_stl_allocator<int8_t>> smaller_node_used_features =
        this->col_sampler_.GetByNode(tree, this->smaller_leaf_splits_->leaf_index());
    std::vector<int8_t, mi_stl_allocator<int8_t>> larger_node_used_features =
