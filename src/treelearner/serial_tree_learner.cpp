...
#pragma omp parallel for schedule(static)
  ...
  Tree *SerialTreeLearner::FitByExistingTree(const Tree *old_tree, const std::vector<int, mi_stl_allocator<int>> &leaf_pred, const score_t *gradients, const score_t *hessians)
  {
  ...
  {
    std::vector<int8_t, mi_stl_allocator<int8_t>> is_feature_used(num_features_, 0);
#pragma omp parallel for schedule(static, 256) if (num_features_ >= 512)
  ...
  void SerialTreeLearner::ConstructHistograms(
      const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used, bool use_subtract)
  {
  ...
  void SerialTreeLearner::FindBestSplitsFromHistograms(
      const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used, bool use_subtract, const Tree *tree)
  {
    Common::FunctionTimer fun_timer(
        "SerialTreeLearner::FindBestSplitsFromHistograms", global_timer);
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> smaller_best(share_state_->num_threads);
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> larger_best(share_state_->num_threads);
    std::vector<int8_t, mi_stl_allocator<int8_t>> smaller_node_used_features = col_sampler_.GetByNode(tree, smaller_leaf_splits_->leaf_index());
...
#pragma omp parallel for schedule(static) num_threads(share_state_->num_threads)
                                     ...
                                     int *right_leaf, bool update_cnt)
  {
    ...
    *left_leaf = best_leaf;
    ...
    {
      std::vector<uint32_t, mi_stl_allocator<uint32_t>> cat_bitset_inner =
          Common::ConstructBitset(best_split_info.cat_threshold.data(),
                                  best_split_info.num_cat_threshold);
      std::vector<int, mi_stl_allocator<int>> threshold_int(best_split_info.num_cat_threshold);
      ...
      std::vector<uint32_t, mi_stl_allocator<uint32_t>> cat_bitset = Common::ConstructBitset(
                                          ...
                                          data_size_t total_num_data, const data_size_t *bag_indices, data_size_t bag_cnt) const
  {
    if (obj != nullptr && obj->IsRenewTreeOutput())
    {
      ...
      std::vector<int, mi_stl_allocator<int>> n_nozeroworker_perleaf(tree->num_leaves(), 1);
...
#pragma omp parallel for schedule(static)
      ...
      if (num_machines > 1)
      {
        std::vector<double, mi_stl_allocator<double>> outputs(tree->num_leaves());
        for (int i = 0; i < tree->num_leaves(); ++i)
        {
  ...
  void SerialTreeLearner::RecomputeBestSplitForLeaf(int leaf, SplitInfo *split)
  {
    ...
    std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> bests(share_state_->num_threads);
