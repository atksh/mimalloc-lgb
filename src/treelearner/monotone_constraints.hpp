...
namespace LightGBM
{
  ...
  public:
    ...
    virtual std::vector<int, mi_stl_allocator<int>> Update(
        ...
        const std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> &best_split_per_leaf) = 0;
  ...
  public:
    ...
    std::vector<int, mi_stl_allocator<int>> Update(const Tree *,
                                                   ...
                                                   const std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> &) override
    {
      ...
      return std::vector<int, mi_stl_allocator<int>>();
    }
  ...
  protected:
    int num_leaves_;
    std::vector<ConstraintEntry, mi_stl_allocator<ConstraintEntry>> entries_;
  };
  ...
  public:
    ...
    std::vector<int, mi_stl_allocator<int>> Update(const Tree *tree, bool is_numerical_split, int leaf,
                                                   int new_leaf, int8_t monotone_type,
                                                   double right_output, double left_output,
                                                   int split_feature, const SplitInfo &split_info,
                                                   const std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> &best_split_per_leaf) override
    {
      leaves_to_update_.clear();
      if (leaf_is_in_monotone_subtree_[leaf])
      {
        ...
        std::vector<int, mi_stl_allocator<int>> features_of_splits_going_up_from_original_leaf;
        std::vector<uint32_t, mi_stl_allocator<uint32_t>> thresholds_of_splits_going_up_from_original_leaf;
        std::vector<bool, mi_stl_allocator<bool>> was_original_leaf_right_child_of_split;
    ...
    bool OppositeChildShouldBeUpdated(
        ...
        const std::vector<int, mi_stl_allocator<int>> &features_of_splits_going_up_from_original_leaf,
        int inner_feature,
        const std::vector<bool, mi_stl_allocator<bool>> &was_original_leaf_right_child_of_split,
    ...
    void GoUpToFindLeavesToUpdate(
        ...
        const std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> &best_split_per_leaf)
    {
#ifdef DEBUG
      CHECK_GE(node_idx, 0);
  ...
  private:
    ...
    std::vector<int, mi_stl_allocator<int>> leaves_to_update_;
    // add parent node information
    std::vector<int, mi_stl_allocator<int>> node_parent_;
    // Keeps track of the monotone splits above the leaf
    std::vector<bool, mi_stl_allocator<bool>> leaf_is_in_monotone_subtree_;
  };
