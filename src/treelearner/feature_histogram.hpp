...
#undef ARGUMENTS
                                           ...
                                           SplitInfo *output)
    {
      ...
      else
      {
        ...
        std::vector<int, mi_stl_allocator<int>> find_direction(1, 1);
        std::vector<int, mi_stl_allocator<int>> start_position(1, 0);
      ...
      if (is_splittable_)
      {
        ...
        if (use_onehot)
        {
          ...
          output->cat_threshold =
              std::vector<uint32_t, mi_stl_allocator<uint32_t>>(1, static_cast<uint32_t>(best_threshold));
        }
        else
        {
          output->num_cat_threshold = best_threshold + 1;
          output->cat_threshold =
              std::vector<uint32_t, mi_stl_allocator<uint32_t>>(output->num_cat_threshold);
          if (best_dir == 1)
          {
                                                ...
                                                SplitInfo *output)
    {
      ...
      output->gain = current_gain - min_gain_shift;
      output->num_cat_threshold = 1;
      output->cat_threshold = std::vector<uint32_t, mi_stl_allocator<uint32_t>>(1, threshold);
    }
   ...
   */
                                              ...
                                              double parent_output)
    {
      ...
      if (USE_SMOOTHING)
      {
        ret = ret * (num_data / smoothing) / (num_data / smoothing + 1) + parent_output / (num_data / smoothing + 1);
      }
      return ret;
    }
   ...
   */
    ...
    static void SetFeatureInfo(const Dataset *train_data, const Config *config,
                               std::vector<FeatureMetainfo, mi_stl_allocator<FeatureMetainfo>> *feature_meta)
    {
...
#pragma omp parallel for schedule(static, 512) if (num_feature >= 1024)
    ...
    static int GetNumTotalHistogramBins(const Dataset *train_data,
                                        bool is_hist_colwise, std::vector<int, mi_stl_allocator<int>> *offsets)
    {
...
#pragma omp parallel for schedule(static)
    ...
    bool Get(int idx, FeatureHistogram **out)
    {
      ...
      else
      {
        ...
        *out = pool_[slot].get();
        ...
        if (inverse_mapper_[slot] >= 0)
          mapper_[inverse_mapper_[slot]] = -1;
  ...
  private:
    ...
    std::vector<int, mi_stl_allocator<int>> mapper_;
    std::vector<int, mi_stl_allocator<int>> inverse_mapper_;
    std::vector<int, mi_stl_allocator<int>> last_used_time_;
    int cur_time_ = 0;
  };
