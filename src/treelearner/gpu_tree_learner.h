...
*/
  ...
  protected:
    ...
    void ConstructHistograms(const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used, bool use_subtract) override;
...
#include "ocl/histogram16.cl"
    ...
    std::vector<boost::compute::kernel, mi_stl_allocator<boost::compute::kernel>> histogram_kernels_;
    /*! \brief an array of histogram kernels with different number
     of workgroups per feature, with all features enabled to avoid branches */
    std::vector<boost::compute::kernel, mi_stl_allocator<boost::compute::kernel>> histogram_allfeats_kernels_;
    /*! \brief an array of histogram kernels with different number
     of workgroups per feature, and processing the whole dataset */
    std::vector<boost::compute::kernel, mi_stl_allocator<boost::compute::kernel>> histogram_fulldata_kernels_;
   ...
   * which GPU kernel to use */
    ...
    std::vector<int, mi_stl_allocator<int>> dense_feature_group_map_;
    /*! \brief Indices of all sparse feature-groups */
    std::vector<int, mi_stl_allocator<int>> sparse_feature_group_map_;
    /*! \brief Multipliers of all dense feature-groups, used for redistributing bins */
    std::vector<int, mi_stl_allocator<int>> device_bin_mults_;
