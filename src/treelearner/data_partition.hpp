...
#pragma omp parallel for schedule(static, 512) if (num_data_ >= 1024)
    ...
    void ResetByLeafPred(const std::vector<int, mi_stl_allocator<int>> &leaf_pred, int num_leaves)
    {
      ResetLeaves(num_leaves);
      std::vector<std::vector<data_size_t, mi_stl_allocator<data_size_t>>> indices_per_leaf(num_leaves_);
  ...
  */
  ...
  private:
    ...
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> leaf_begin_;
    /*! \brief number of data on one leaf */
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> leaf_count_;
