...
#pragma omp parallel for schedule(static, 512) if (omp_loop_size >= 1024)
    ...
    std::vector<int8_t, mi_stl_allocator<int8_t>> GetByNode(const Tree *tree, int leaf)
    {
      // get interaction constraints for current branch
      std::unordered_set<int> allowed_features;
      if (!interaction_constraints_.empty())
      {
        std::vector<int, mi_stl_allocator<int>> branch_features = tree->branch_features(leaf);
      ...
      std::vector<int8_t, mi_stl_allocator<int8_t>> ret(train_data_->num_features(), 0);
      if (fraction_bynode_ >= 1.0f)
      {
        if (interaction_constraints_.empty())
        {
          return std::vector<int8_t, mi_stl_allocator<int8_t>>(train_data_->num_features(), 1);
        }
        else
        {
      ...
      if (need_reset_bytree_)
      {
        ...
        std::vector<int, mi_stl_allocator<int>> *allowed_used_feature_indices;
        std::vector<int, mi_stl_allocator<int>> filtered_feature_indices;
        if (interaction_constraints_.empty())
        {
...
#pragma omp parallel for schedule(static, 512) if (omp_loop_size >= 1024)
      ...
      else
      {
        ...
        std::vector<int, mi_stl_allocator<int>> *allowed_valid_feature_indices;
        std::vector<int, mi_stl_allocator<int>> filtered_feature_indices;
        if (interaction_constraints_.empty())
        {
...
#pragma omp parallel for schedule(static, 512) if (omp_loop_size >= 1024)
    ...
    const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used_bytree() const
    {
  ...
  private:
    ...
    int seed_;
    Random random_;
    std::vector<int8_t, mi_stl_allocator<int8_t>> is_feature_used_;
    std::vector<int, mi_stl_allocator<int>> used_feature_indices_;
    std::vector<int, mi_stl_allocator<int>> valid_feature_indices_;
