...
*/
  ...
  protected:
    ...
    void FindBestSplitsFromHistograms(const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used, bool use_subtract, const Tree *tree) override;
  ...
  private:
    ...
    std::vector<char, mi_stl_allocator<char>> input_buffer_;
    /*! \brief Buffer for network receive */
    std::vector<char, mi_stl_allocator<char>> output_buffer_;
  };
...
*/
  ...
  private:
    ...
    std::vector<bool, mi_stl_allocator<bool>> is_feature_aggregated_;
    /*! \brief Block start index for reduce scatter */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_start_;
    /*! \brief Block size for reduce scatter */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_len_;
    /*! \brief Write positions for feature histograms */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> buffer_write_start_pos_;
    /*! \brief Read positions for local feature histograms */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> buffer_read_start_pos_;
    /*! \brief Size for reduce scatter */
    comm_size_t reduce_scatter_size_;
    /*! \brief Store global number of data in leaves  */
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> global_data_count_in_leaf_;
  };
...
*/
  ...
  */
    void GlobalVoting(int leaf_idx, const std::vector<LightSplitInfo, mi_stl_allocator<LightSplitInfo>> &splits,
                      std::vector<int, mi_stl_allocator<int>> *out);
    /*!
  ...
  */
    void CopyLocalHistogram(const std::vector<int, mi_stl_allocator<int>> &smaller_top_features,
                            const std::vector<int, mi_stl_allocator<int>> &larger_top_features);
  ...
  private:
    ...
    /*! \brief different machines will aggregate histograms for different features,
  use this to mark local aggregate features*/
    std::vector<bool, mi_stl_allocator<bool>> smaller_is_feature_aggregated_;
    /*! \brief different machines will aggregate histograms for different features,
  use this to mark local aggregate features*/
    std::vector<bool, mi_stl_allocator<bool>> larger_is_feature_aggregated_;
    /*! \brief Block start index for reduce scatter */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_start_;
    /*! \brief Block size for reduce scatter */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_len_;
    /*! \brief Read positions for feature histgrams at smaller leaf */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> smaller_buffer_read_start_pos_;
    /*! \brief Read positions for feature histgrams at larger leaf */
    std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> larger_buffer_read_start_pos_;
    /*! \brief Size for reduce scatter */
    comm_size_t reduce_scatter_size_;
    /*! \brief Store global number of data in leaves  */
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> global_data_count_in_leaf_;
    ...
    std::vector<hist_t, mi_stl_allocator<hist_t>> smaller_leaf_histogram_data_;
    std::vector<hist_t, mi_stl_allocator<hist_t>> larger_leaf_histogram_data_;
    std::vector<FeatureMetainfo, mi_stl_allocator<FeatureMetainfo>> feature_metas_;
  };
