...
namespace LightGBM
{
  ...
  public:
    ...
    void UpdateLeafBestSplits(Tree *tree, int best_leaf,
                              ...
                              std::vector<SplitInfo, mi_stl_allocator<SplitInfo>> *best_split_per_leaf)
    {
      auto config = tree_learner_->config_;
      auto train_data = tree_learner_->train_data_;
  ...
  private:
    ...
    std::vector<bool, mi_stl_allocator<bool>> is_feature_used_in_split_;
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> feature_used_in_data_;
  };
