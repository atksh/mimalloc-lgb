...
*/
  ...
  */
              ...
              int early_stop_freq, double early_stop_margin)
    {
      ...
      {
        predict_fun_ = [=](const std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &features,
                           double *output) {
      ...
      else if (predict_contrib)
      {
        ...
        predict_sparse_fun_ = [=](const std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &features,
                                  std::vector<std::unordered_map<int, double>, mi_stl_allocator<std::unordered_map<int, double>>> *output) {
          auto buf = CopyToPredictMap(features);
  ...
  */
    void Predict(const char *data_filename, const char *result_filename, bool header, bool disable_shape_check)
    {
      ...
      std::vector<int, mi_stl_allocator<int>> feature_remapper(parser->NumFeatures(), -1);
      bool need_adjust = false;
      if (header)
      {
        std::string first_line = predict_data_reader.first_line();
        std::vector<std::string, mi_stl_allocator<std::string>> header_words = Common::Split(first_line.c_str(), "\t,");
      ...
      std::function<void(const char *, std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> *)> parser_fun;
      double tmp_label;
      parser_fun = [&parser, &feature_remapper, &tmp_label, need_adjust](const char *buffer, std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> *feature) {
      ...
      std::function<void(data_size_t, const std::vector<std::string, mi_stl_allocator<std::string>> &)>
          process_fun = [&parser_fun, &writer, this](
                            data_size_t, const std::vector<std::string, mi_stl_allocator<std::string>> &lines) {
            std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> oneline_features;
            std::vector<std::string, mi_stl_allocator<std::string>> result_to_write(lines.size());
...
#pragma omp parallel for schedule(static) firstprivate(oneline_features)
            for (data_size_t i = 0; i < static_cast<data_size_t>(lines.size()); ++i)
            {
              ...
              // predict
              std::vector<double, mi_stl_allocator<double>> result(num_pred_one_row_);
  ...
  private:
    void CopyToPredictBuffer(double *pred_buf, const std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &features)
    {
    ...
    void ClearPredictBuffer(double *pred_buf, size_t buf_size, const std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &features)
    {
    ...
    std::unordered_map<int, double> CopyToPredictMap(const std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &features)
    {
