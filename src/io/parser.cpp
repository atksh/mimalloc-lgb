...
namespace LightGBM
{
  ...
  void GetLine(std::stringstream *ss, std::string *line, const VirtualFileReader *reader, std::vector<char, mi_stl_allocator<char>> *buffer, size_t buffer_size)
  {
    std::getline(*ss, *line);
    while (ss->eof())
    {
  ...
  std::vector<std::string, mi_stl_allocator<std::string>> ReadKLineFromFile(const char *filename, bool header, int k)
  {
  ...
  DataType GetDataType(const char *filename, bool header,
                       const std::vector<std::string, mi_stl_allocator<std::string>> &lines, int *num_col)
  {
