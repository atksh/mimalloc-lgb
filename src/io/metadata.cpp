...
namespace LightGBM
{
  ...
  {
    num_data_ = num_data;
    label_ = std::vector<label_t, mi_stl_allocator<label_t>>(num_data_);
    if (weight_idx >= 0)
    {
      if (!weights_.empty())
      {
      ...
      weights_ = std::vector<label_t, mi_stl_allocator<label_t>>(num_data_, 0.0f);
    ...
    if (query_idx >= 0)
    {
      ...
      queries_ = std::vector<data_size_t, mi_stl_allocator<data_size_t>>(num_data_, 0);
      query_load_from_file_ = false;
    }
  }
  ...
  void Metadata::Init(const Metadata &fullset, const data_size_t *used_indices, data_size_t num_used_indices)
  {
    ...
    label_ = std::vector<label_t, mi_stl_allocator<label_t>>(num_used_indices);
#pragma omp parallel for schedule(static, 512) if (num_used_indices >= 1024)
    ...
    {
      weights_ = std::vector<label_t, mi_stl_allocator<label_t>>(num_used_indices);
...
#pragma omp parallel for schedule(static, 512) if (num_used_indices >= 1024)
    ...
    if (!fullset.init_score_.empty())
    {
      ...
      init_score_ = std::vector<double, mi_stl_allocator<double>>(static_cast<size_t>(num_used_indices) * num_class);
...
#pragma omp parallel for schedule(static)
    ...
    {
      std::vector<data_size_t, mi_stl_allocator<data_size_t>> used_query;
      ...
      query_boundaries_ = std::vector<data_size_t, mi_stl_allocator<data_size_t>>(used_query.size() + 1);
  ...
  void Metadata::PartitionLabel(const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &used_indices)
  {
...
#pragma omp parallel for schedule(static, 512) if (num_data_ >= 1024)
  ...
  void Metadata::CheckOrPartition(data_size_t num_all_data, const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &used_data_indices)
  {
    if (used_data_indices.empty())
    {
      if (!queries_.empty())
      {
        ...
        std::vector<data_size_t, mi_stl_allocator<data_size_t>> tmp_buffer;
        ...
        tmp_buffer.push_back(cur_cnt);
        query_boundaries_ = std::vector<data_size_t, mi_stl_allocator<data_size_t>>(tmp_buffer.size() + 1);
    ...
    else
    {
      ...
      if (weight_load_from_file_)
      {
        ...
        if (!weights_.empty())
        {
          ...
          num_weights_ = num_data_;
          weights_ = std::vector<label_t, mi_stl_allocator<label_t>>(num_data_);
#pragma omp parallel for schedule(static, 512)
      ...
      if (init_score_load_from_file_)
      {
        ...
        if (!init_score_.empty())
        {
          ...
          init_score_ = std::vector<double, mi_stl_allocator<double>>(num_init_score_);
#pragma omp parallel for schedule(static)
          for (int k = 0; k < num_class; ++k)
          {
            const size_t offset_dest = static_cast<size_t>(k) * num_data_;
...
#pragma omp parallel for schedule(static) reduction(+ \
  ...
  void Metadata::LoadWeights()
  {
    ...
    weights_ = std::vector<label_t, mi_stl_allocator<label_t>>(num_weights_);
#pragma omp parallel for schedule(static)
    for (data_size_t i = 0; i < num_weights_; ++i)
    {
...
#pragma omp parallel for schedule(static)
    ...
    {
      std::vector<std::string, mi_stl_allocator<std::string>> oneline_init_score;
#pragma omp parallel for schedule(static)
      for (data_size_t i = 0; i < num_line; ++i)
      {
        double tmp = 0.0f;
  ...
  void Metadata::LoadQueryBoundaries()
  {
    ...
    query_boundaries_ = std::vector<data_size_t, mi_stl_allocator<data_size_t>>(reader.Lines().size() + 1);
  ...
  void Metadata::LoadQueryWeights()
  {
    ...
    query_weights_ = std::vector<label_t, mi_stl_allocator<label_t>>(num_queries_);
    for (data_size_t i = 0; i < num_queries_; ++i)
    {
  ...
  void Metadata::LoadFromMemory(const void *memory)
  {
    ...
    if (num_queries_ > 0)
    {
      ...
      query_boundaries_ = std::vector<data_size_t, mi_stl_allocator<data_size_t>>(num_queries_ + 1);
  ...
  {
    size_t size = sizeof(num_data_) + sizeof(num_weights_) + sizeof(num_queries_);
