...
namespace LightGBM
{
  ...
  bool NeedFilter(const std::vector<int, mi_stl_allocator<int>> &cnt_in_bin, int total_cnt, int filter_cnt, BinType bin_type)
  {
  ...
  std::vector<double, mi_stl_allocator<double>> GreedyFindBin(const double *distinct_values, const int *counts,
                                                              int num_distinct_values, int max_bin,
                                                              size_t total_cnt, int min_data_in_bin)
  {
    std::vector<double, mi_stl_allocator<double>> bin_upper_bound;
    CHECK_GT(max_bin, 0);
    ...
    else
    {
      ...
      std::vector<bool, mi_stl_allocator<bool>> is_big_count_value(num_distinct_values, false);
      ...
      std::vector<double, mi_stl_allocator<double>> upper_bounds(max_bin, std::numeric_limits<double>::infinity());
      std::vector<double, mi_stl_allocator<double>> lower_bounds(max_bin, std::numeric_limits<double>::infinity());
  ...
  std::vector<double, mi_stl_allocator<double>> FindBinWithPredefinedBin(const double *distinct_values, const int *counts,
                                                                         int num_distinct_values, int max_bin,
                                                                         size_t total_sample_cnt, int min_data_in_bin,
                                                                         const std::vector<double, mi_stl_allocator<double>> &forced_upper_bounds)
  {
    std::vector<double, mi_stl_allocator<double>> bin_upper_bound;
    ...
    std::vector<double, mi_stl_allocator<double>> bounds_to_add;
    int value_ind = 0;
    for (size_t i = 0; i < bin_upper_bound.size(); ++i)
    {
      ...
      std::vector<double, mi_stl_allocator<double>> new_upper_bounds = GreedyFindBin(distinct_values + bin_start, counts + bin_start, distinct_cnt_in_bin,
  ...
  std::vector<double, mi_stl_allocator<double>> FindBinWithZeroAsOneBin(const double *distinct_values, const int *counts, int num_distinct_values,
                          ...
                          const std::vector<double, mi_stl_allocator<double>> &forced_upper_bounds)
  {
    ...
    std::vector<double, mi_stl_allocator<double>> distinct_values;
    std::vector<int, mi_stl_allocator<int>> counts;
    ...
    std::vector<int, mi_stl_allocator<int>> cnt_in_bin;
    ...
    else
    {
      ...
      std::vector<int, mi_stl_allocator<int>> distinct_values_int;
      std::vector<int, mi_stl_allocator<int>> counts_int;
      ...
      if (rest_cnt > 0)
      {
        ...
        cnt_in_bin.clear();
        while (cur_cat < distinct_values_int.size() && (used_cnt < cut_cnt || num_bin_ < max_bin))
        {
  ...
  void BinMapper::CopyFrom(const char *buffer)
  {
    ...
    if (bin_type_ == BinType::NumericalBin)
    {
      bin_upper_bound_ = std::vector<double, mi_stl_allocator<double>>(num_bin_);
      std::memcpy(bin_upper_bound_.data(), buffer, num_bin_ * sizeof(double));
    }
    else
    {
      bin_2_categorical_ = std::vector<int, mi_stl_allocator<int>>(num_bin_);
  ...
  {
    size_t ret = sizeof(num_bin_) + sizeof(missing_type_) + sizeof(is_trivial_) + sizeof(sparse_rate_) + sizeof(bin_type_) + sizeof(min_val_) + sizeof(max_val_) + sizeof(default_bin_) + sizeof(most_freq_bin_);
    if (bin_type_ == BinType::NumericalBin)
    {
