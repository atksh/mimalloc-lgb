...
namespace LightGBM
{
  ...
  Dataset *DatasetLoader::LoadFromFile(const char *filename, int rank, int num_machines)
  {
    ...
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> used_data_indices;
                                          ...
                                          std::vector<data_size_t, mi_stl_allocator<data_size_t>> *used_data_indices)
  {
    ...
    auto buffer = std::vector<char, mi_stl_allocator<char>>(buffer_size);
    ...
    dataset->forced_bin_bounds_ = std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>>(dataset->num_total_features_, std::vector<double, mi_stl_allocator<double>>());
    for (int i = 0; i < dataset->num_total_features_; ++i)
    {
      int num_bounds = *(reinterpret_cast<const int *>(mem_ptr));
      mem_ptr += sizeof(int);
      dataset->forced_bin_bounds_[i] = std::vector<double, mi_stl_allocator<double>>();
                                                  ...
                                                  size_t total_sample_size, data_size_t num_data)
  {
    ...
    std::vector<std::unique_ptr<BinMapper>, mi_stl_allocator<std::unique_ptr<BinMapper>>> bin_mappers(num_total_features);
    ...
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> forced_bin_bounds = DatasetLoader::GetForcedBins(forced_bins_path, num_col, categorical_features_);
...
#pragma omp parallel for schedule(guided)
    ...
    else
    {
      ...
      std::vector<int, mi_stl_allocator<int>> start(num_machines);
      std::vector<int, mi_stl_allocator<int>> len(num_machines);
...
#pragma omp parallel for schedule(guided)
      ...
      std::vector<char, mi_stl_allocator<char>> input_buffer(self_buf_size);
      ...
      std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> size_len = Network::GlobalArray(self_buf_size);
      std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> size_start(num_machines, 0);
      ...
      std::vector<char, mi_stl_allocator<char>> output_buffer(total_buffer_size);
  ...
  std::vector<std::string, mi_stl_allocator<std::string>> DatasetLoader::LoadTextDataToMemory(const char *filename, const Metadata &metadata,
                                                                                              int rank, int num_machines, int *num_global_data,
                                                                                              std::vector<data_size_t, mi_stl_allocator<data_size_t>> *used_data_indices)
  {
    ...
    { // need partition data
      ...
      else
      {
        ...
        *num_global_data = text_reader.ReadAndFilterLines(
            [this, rank, num_machines, &qid, &query_boundaries, &is_query_used, num_queries](data_size_t line_idx) {
              if (qid >= num_queries)
              {
                Log::Fatal("Current query exceeds the range of the query file,\n"
                           "please ensure the query file is correct");
  ...
  std::vector<std::string, mi_stl_allocator<std::string>> DatasetLoader::SampleTextDataFromMemory(const std::vector<std::string, mi_stl_allocator<std::string>> &data)
  {
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> out(sample_indices.size());
  ...
  std::vector<std::string, mi_stl_allocator<std::string>> DatasetLoader::SampleTextDataFromFile(const char *filename, const Metadata &metadata,
                                                                                                int rank, int num_machines, int *num_global_data,
                                                                                                std::vector<data_size_t, mi_stl_allocator<data_size_t>> *used_data_indices)
  {
    const data_size_t sample_cnt = static_cast<data_size_t>(config_.bin_construct_sample_cnt);
    TextReader<data_size_t> text_reader(filename, config_.header, config_.file_load_progress_interval_bytes);
    std::vector<std::string, mi_stl_allocator<std::string>> out_data;
    if (num_machines == 1 || config_.pre_partition)
    {
    ...
    { // need partition data
      ...
      {
        // if not contain query file, minimal sample unit is one record
        *num_global_data = text_reader.SampleAndFilterFromFile([this, rank, num_machines](data_size_t) {
          if (random_.NextShort(0, num_machines) == rank)
          {
            return true;
          }
  ...
  void DatasetLoader::ConstructBinMappersFromTextData(int rank, int num_machines,
                                                      const std::vector<std::string, mi_stl_allocator<std::string>> &sample_data,
                                                      const Parser *parser, Dataset *dataset)
  {
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> sample_values;
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> sample_indices;
    std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> oneline_features;
    double label;
    ...
    // get forced split
    std::string forced_bins_path = config_.forcedbins_filename;
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> forced_bin_bounds = DatasetLoader::GetForcedBins(forced_bins_path,
    ...
    dataset->set_feature_names(feature_names_);
    std::vector<std::unique_ptr<BinMapper>, mi_stl_allocator<std::unique_ptr<BinMapper>>> bin_mappers(dataset->num_total_features_);
    const data_size_t filter_cnt = static_cast<data_size_t>(
...
#pragma omp parallel for schedule(guided)
  ...
  void DatasetLoader::ExtractFeaturesFromMemory(std::vector<std::string, mi_stl_allocator<std::string>> *text_data, const Parser *parser, Dataset *dataset)
  {
    std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> oneline_features;
...
#pragma omp parallel for schedule(static) private(oneline_features) firstprivate(tmp_label)
      for (data_size_t i = 0; i < dataset->num_data_; ++i)
      {
        ...
        std::vector<bool, mi_stl_allocator<bool>> is_feature_added(dataset->num_features_, false);
        // push data
    ...
    else
    {
      ...
      std::vector<double, mi_stl_allocator<double>> init_score(dataset->num_data_ * num_class_);
#pragma omp parallel for schedule(static) private(oneline_features) firstprivate(tmp_label)
      for (data_size_t i = 0; i < dataset->num_data_; ++i)
      {
        OMP_LOOP_EX_BEGIN();
        ...
        parser->ParseOneLine(ref_text_data[i].c_str(), &oneline_features, &tmp_label);
        // set initial score
        std::vector<double, mi_stl_allocator<double>> oneline_init_score(num_class_);
  ...
  void DatasetLoader::ExtractFeaturesFromFile(const char *filename, const Parser *parser,
                                              const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &used_data_indices, Dataset *dataset)
  {
    std::vector<double, mi_stl_allocator<double>> init_score;
    if (predict_fun_ != nullptr)
    {
      init_score = std::vector<double, mi_stl_allocator<double>>(dataset->num_data_ * num_class_);
    }
    std::function<void(data_size_t, const std::vector<std::string, mi_stl_allocator<std::string>> &)> process_fun =
        [this, &init_score, &parser, &dataset](data_size_t start_idx, const std::vector<std::string, mi_stl_allocator<std::string>> &lines) {
          std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> oneline_features;
          double tmp_label = 0.0f;
          OMP_INIT_EX();
#pragma omp parallel for schedule(static) private(oneline_features) firstprivate(tmp_label)
  ...
  std::string DatasetLoader::CheckCanLoadFromBin(const char *filename)
  {
    ...
    if (read_cnt == size_of_token && std::string(buffer.data()) == std::string(Dataset::binary_file_token))
    {
      return bin_filename;
    }
    else
  ...
  std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> DatasetLoader::GetForcedBins(std::string forced_bins_path, int num_total_features,
                                                                                                                                                           const std::unordered_set<int> &categorical_features)
  {
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> forced_bins(num_total_features, std::vector<double, mi_stl_allocator<double>>());
    if (forced_bins_path != "")
    {
      ...
      else
      {
        ...
        std::vector<Json, mi_stl_allocator<Json>> forced_bins_arr = forced_bins_json.array_items();
        for (size_t i = 0; i < forced_bins_arr.size(); ++i)
        {
          ...
          {
            std::vector<Json, mi_stl_allocator<Json>> bounds_arr = forced_bins_arr[i]["bin_upper_bound"].array_items();
