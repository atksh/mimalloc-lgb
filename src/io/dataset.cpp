...
namespace LightGBM
{
  ...
  std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> NoGroup(const std::vector<int, mi_stl_allocator<int>> &used_features)
  {
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> features_in_group;
  ...
  int GetConflictCount(const std::vector<bool, mi_stl_allocator<bool>> &mark, const int *indices,
  ...
  void MarkUsed(std::vector<bool, mi_stl_allocator<bool>> *mark, const int *indices,
  ...
  auto FixSampleIndices(const BinMapper *bin_mapper,
  ...
  std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> FindGroups(
      const std::vector<std::unique_ptr<BinMapper>, mi_stl_allocator<std::unique_ptr<BinMapper>>> &bin_mappers,
      const std::vector<int, mi_stl_allocator<int>> &find_order, int **sample_indices,
      const int *num_per_col, int num_sample_col, data_size_t total_sample_cnt,
      data_size_t num_data, bool is_use_gpu, bool is_sparse,
      std::vector<int8_t, mi_stl_allocator<int8_t>> *multi_val_group)
  {
    ...
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> features_in_group;
    std::vector<std::vector<bool, mi_stl_allocator<bool>>, mi_stl_allocator<std::vector<bool, mi_stl_allocator<bool>>>> conflict_marks;
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> group_used_row_cnt;
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> group_total_data_cnt;
    std::vector<int, mi_stl_allocator<int>> group_num_bin;
    ...
    for (auto fidx : find_order)
    {
      ...
      std::vector<int, mi_stl_allocator<int>> available_groups;
      ...
      std::vector<int, mi_stl_allocator<int>> search_groups;
    ...
    std::vector<int, mi_stl_allocator<int>> second_round_features;
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> features_in_group2;
    std::vector<std::vector<bool, mi_stl_allocator<bool>>, mi_stl_allocator<std::vector<bool, mi_stl_allocator<bool>>>> conflict_marks2;
  ...
  std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> FastFeatureBundling(
      const std::vector<std::unique_ptr<BinMapper>, mi_stl_allocator<std::unique_ptr<BinMapper>>> &bin_mappers,
      int **sample_indices, double **sample_values, const int *num_per_col,
      int num_sample_col, data_size_t total_sample_cnt,
      const std::vector<int, mi_stl_allocator<int>> &used_features, data_size_t num_data,
      bool is_use_gpu, bool is_sparse, std::vector<int8_t, mi_stl_allocator<int8_t>> *multi_val_group)
  {
    Common::FunctionTimer fun_timer("Dataset::FastFeatureBundling", global_timer);
    std::vector<size_t, mi_stl_allocator<size_t>> feature_non_zero_cnt;
    ...
    std::vector<int, mi_stl_allocator<int>> sorted_idx;
    ...
    std::vector<int, mi_stl_allocator<int>> feature_order_by_cnt;
    ...
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> tmp_indices;
    std::vector<int, mi_stl_allocator<int>> tmp_num_per_col(num_sample_col, 0);
    for (auto fidx : used_features)
    {
      if (fidx >= num_sample_col)
      {
    ...
    std::vector<int8_t, mi_stl_allocator<int8_t>> group_is_multi_val, group_is_multi_val2;
  ...
  void Dataset::Construct(std::vector<std::unique_ptr<BinMapper>, mi_stl_allocator<std::unique_ptr<BinMapper>>> *bin_mappers,
                          int num_total_features,
                          const std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> &forced_bins,
                          ...
                          size_t total_sample_cnt, const Config &io_config)
  {
    ...
    // get num_features
    std::vector<int, mi_stl_allocator<int>> used_features;
    ...
    std::vector<int8_t, mi_stl_allocator<int8_t>> group_is_multi_val(used_features.size(), 0);
    ...
    int cur_fidx = 0;
    used_feature_map_ = std::vector<int, mi_stl_allocator<int>>(num_total_features_, -1);
    ...
    for (int i = 0; i < num_groups_; ++i)
    {
      ...
      // get bin_mappers
      std::vector<std::unique_ptr<BinMapper>, mi_stl_allocator<std::unique_ptr<BinMapper>>> cur_bin_mappers;
  ...
  void PushDataToMultiValBin(
      data_size_t num_data, const std::vector<uint32_t, mi_stl_allocator<uint32_t>> most_freq_bins,
      const std::vector<uint32_t, mi_stl_allocator<uint32_t>> offsets,
      std::vector<std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>>, mi_stl_allocator<std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>>>> *iters,
      MultiValBin *ret)
  {
    ...
    if (ret->IsSparse())
    {
      Threading::For<data_size_t>(
          0, num_data, 1024, [&](int tid, data_size_t start, data_size_t end) {
            std::vector<uint32_t, mi_stl_allocator<uint32_t>> cur_data;
    ...
    else
    {
      Threading::For<data_size_t>(
          0, num_data, 1024, [&](int tid, data_size_t start, data_size_t end) {
            std::vector<uint32_t, mi_stl_allocator<uint32_t>> cur_data(most_freq_bins.size(), 0);
            for (size_t j = 0; j < most_freq_bins.size(); ++j)
            {
              (*iters)[tid][j]->Reset(start);
            }
  ...
  MultiValBin *Dataset::GetMultiBinFromSparseFeatures() const
  {
    ...
    std::vector<std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>>, mi_stl_allocator<std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>>>> iters(num_threads);
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> most_freq_bins;
...
#pragma omp parallel for schedule(static, 1)
  ...
  MultiValBin *Dataset::GetMultiBinFromAllFeatures() const
  {
    ...
    std::vector<std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>>, mi_stl_allocator<std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>>>> iters(num_threads);
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> most_freq_bins;
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> offsets;
    int num_total_bin = 1;
...
#pragma omp parallel for schedule(static, 1)
  ...
  TrainingShareStates *Dataset::GetShareStates(
      ...
      const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used, bool is_constant_hessian,
...
#pragma omp parallel for schedule(static)
  ...
  void Dataset::SaveBinaryFile(const char *bin_filename)
  {
    ...
    if (!is_file_existed)
    {
      ...
      // get size of header
      size_t size_of_header = sizeof(num_data_) + sizeof(num_features_) + sizeof(num_total_features_) + sizeof(int) * num_total_features_ + sizeof(label_idx_) + sizeof(num_groups_) + 3 * sizeof(int) * num_features_ + sizeof(uint64_t) * (num_groups_ + 1) + 2 * sizeof(int) * num_groups_ + sizeof(int32_t) * num_total_features_ + sizeof(int) * 3 + sizeof(bool) * 2;
  ...
  void Dataset::DumpTextFile(const char *text_filename)
  {
    ...
    std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>> iterators;
  ...
  void Dataset::InitTrain(const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used,
                          TrainingShareStates *share_state) const
  {
    ...
    int num_used = 0;
    int total = 0;
    std::vector<int, mi_stl_allocator<int>> used_feature_index;
    for (int i = 0; i < num_groups_; ++i)
    {
    ...
    else
    {
      ...
      std::vector<uint32_t, mi_stl_allocator<uint32_t>> upper_bound;
      std::vector<uint32_t, mi_stl_allocator<uint32_t>> lower_bound;
      std::vector<uint32_t, mi_stl_allocator<uint32_t>> delta;
...
#pragma omp parallel for schedule(static, 1) num_threads(share_state->num_threads)
  ...
  void Dataset::ConstructHistogramsInner(
      const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used, const data_size_t *data_indices,
      ...
      TrainingShareStates *share_state, hist_t *hist_data) const
  {
    ...
    std::vector<int, mi_stl_allocator<int>> used_dense_group;
...
#pragma omp parallel for schedule(static) num_threads(share_state->num_threads)
  ...
  template <typename T>
  void PushVector(std::vector<T, mi_stl_allocator<T>> *dest, const std::vector<T, mi_stl_allocator<T>> &src)
  {
  ...
  template <typename T>
  void PushOffset(std::vector<T, mi_stl_allocator<T>> *dest, const std::vector<T, mi_stl_allocator<T>> &src,
                  const T &offset)
  {
    dest->reserve(dest->size() + src.size());
    for (auto i : src)
  ...
  template <typename T>
  void PushClearIfEmpty(std::vector<T, mi_stl_allocator<T>> *dest, const size_t dest_len,
                        const std::vector<T, mi_stl_allocator<T>> &src, const size_t src_len,
                        const T &deflt)
  {
