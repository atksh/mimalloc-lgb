...
namespace LightGBM
{
  ...
  {
    std::vector<std::string, mi_stl_allocator<std::string>> tmp_strs = Common::Split(kv, '=');
  ...
  void ParseMetrics(const std::string &value, std::vector<std::string, mi_stl_allocator<std::string>> *out_metric)
  {
    std::unordered_set<std::string> metric_sets;
    out_metric->clear();
    std::vector<std::string, mi_stl_allocator<std::string>> metrics = Common::Split(value.c_str(), ',');
  ...
  void GetMetricType(const std::unordered_map<std::string, std::string> &params, std::vector<std::string, mi_stl_allocator<std::string>> *metric)
  {
    std::string value;
  ...
  void Config::GetAucMuWeights()
  {
    if (auc_mu_weights.empty())
    {
      ...
      auc_mu_weights_matrix = std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>>(num_class, std::vector<double, mi_stl_allocator<double>>(num_class, 1));
    ...
    {
      auc_mu_weights_matrix = std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>>(num_class, std::vector<double, mi_stl_allocator<double>>(num_class, 0));
  ...
  void Config::GetInteractionConstraints()
  {
    ...
    {
      interaction_constraints_vector = std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>>();
    }
    else
    {
  ...
  void Config::Set(const std::unordered_map<std::string, std::string> &params)
  {
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> new_valid;
  ...
  void Config::CheckParamConflict()
  {
    ...
    {
      bool metric_type_multiclass = (CheckMultiClassObjective(metric_type) || metric_type == std::string("multi_logloss") || metric_type == std::string("multi_error") || metric_type == std::string("auc_mu") || (metric_type == std::string("custom") && num_class_check > 1));
