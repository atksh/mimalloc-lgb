...
namespace LightGBM
{
      ...
      : max_leaves_(max_leaves), track_branch_features_(track_branch_features)
  {
    ...
    {
      branch_features_ = std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>>(max_leaves_);
    }
...
#define PredictionFun(niter, fidx_in_iter, start_pos, decision_fun, iter_idx,                              \
                      data_idx)                                                                            \
  std::vector<std::unique_ptr<BinIterator>, mi_stl_allocator<std::unique_ptr<BinIterator>>> iter((niter)); \
...
#pragma omp parallel for schedule(static, 512) if (num_data >= 1024)
    ...
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> default_bins(num_leaves_ - 1);
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> max_bins(num_leaves_ - 1);
    ...
    if (num_cat_ > 0)
    {
      ...
      {
        Threading::For<data_size_t>(0, num_data, 512, [this, &data, score, &default_bins, &max_bins](int, data_size_t start, data_size_t end) {
...
#pragma omp parallel for schedule(static, 512) if (num_data >= 1024)
    ...
    if (num_cat_ > 0)
    {
      if (data->num_features() > num_leaves_ - 1)
      {
        Threading::For<data_size_t>(0, num_data, 512, [this, &data, score, used_data_indices, &default_bins, &max_bins](int, data_size_t start, data_size_t end) {
...
#undef PredictionFun
  ...
  std::string Tree::NodeToJSON(int index) const
  {
    ...
    if (index >= 0)
    {
      ...
      if (GetDecisionType(decision_type_[index], kCategoricalMask))
      {
        ...
        std::vector<int, mi_stl_allocator<int>> cats;
  ...
  std::string Tree::NumericalDecisionIfElse(int node) const
  {
    ...
    if (missing_type == MissingType::None || (missing_type == MissingType::Zero && default_left && kZeroThreshold < threshold_[node]))
    {
  ...
  std::string Tree::ToIfElse(int index, bool predict_leaf_index) const
  {
    ...
    {
      str_buf << "const std::vector<uint32_t, mi_stl_allocator<uint32_t>> cat_threshold = {";
  ...
  Tree::Tree(const char *str, size_t *used_len)
  {
    ...
    *used_len = p - str;
    ...
    {
      decision_type_ = std::vector<int8_t, mi_stl_allocator<int8_t>>(num_leaves_ - 1, 0);
    }
                        ...
                        double zero_fraction, double one_fraction, int feature_index)
  {
    ...
    {
      unique_path[i + 1].pweight += one_fraction * unique_path[i].pweight * (i + 1) / static_cast<double>(unique_depth + 1);
      unique_path[i].pweight = zero_fraction * unique_path[i].pweight * (unique_depth - i) / static_cast<double>(unique_depth + 1);
    }
  }
  ...
  void Tree::UnwindPath(PathElement *unique_path, int unique_depth, int path_index)
  {
    ...
    for (int i = unique_depth - 1; i >= 0; --i)
    {
      if (one_fraction != 0)
      {
        ...
        unique_path[i].pweight = next_one_portion * (unique_depth + 1) / static_cast<double>((i + 1) * one_fraction);
        next_one_portion = tmp - unique_path[i].pweight * zero_fraction * (unique_depth - i) / static_cast<double>(unique_depth + 1);
      }
      else
      {
        unique_path[i].pweight = (unique_path[i].pweight * (unique_depth + 1)) / static_cast<double>(zero_fraction * (unique_depth - i));
      }
    }
  ...
  double Tree::UnwoundPathSum(const PathElement *unique_path, int unique_depth, int path_index)
  {
    ...
    for (int i = unique_depth - 1; i >= 0; --i)
    {
      if (one_fraction != 0)
      {
        const double tmp = next_one_portion * (unique_depth + 1) / static_cast<double>((i + 1) * one_fraction);
        total += tmp;
        next_one_portion = unique_path[i].pweight - tmp * zero_fraction * ((unique_depth - i) / static_cast<double>(unique_depth + 1));
      }
      else
      {
        total += (unique_path[i].pweight / zero_fraction) / ((unique_depth - i) / static_cast<double>(unique_depth + 1));
      }
    }
    return total;
  }
                      ...
                      double parent_one_fraction, int parent_feature_index) const
  {
    ...
    else
    {
      ...
      {
        if (unique_path[path_index].feature_index == split_feature_[node])
          break;
      }
  ...
  void Tree::RecomputeMaxDepth()
  {
    ...
    else
    {
      ...
      {
        if (max_depth_ < leaf_depth_[i])
          max_depth_ = leaf_depth_[i];
      }
    }
  }
