...
namespace LightGBM
{
  ...
  public:
    ...
    void PushOneRow(int tid, data_size_t idx,
                    const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &values) override
    {
    ...
    void MergeData(const INDEX_T *sizes)
    {
      ...
      if (t_data_.size() > 0)
      {
        std::vector<INDEX_T, mi_stl_allocator<INDEX_T>> offsets(1 + t_data_.size());
        offsets[0] = sizes[0];
...
#pragma omp parallel for schedule(static, 1)
                   ...
                   const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &delta)
    {
      ...
      std::vector<INDEX_T, mi_stl_allocator<INDEX_T>> sizes(t_data_.size() + 1, 0);
      const int pre_alloc_size = 50;
#pragma omp parallel for schedule(static, 1)
                    ...
                    data_size_t num_used_indices) override
    {
      CopyInner<true, false>(full_bin, used_indices, num_used_indices,
                             std::vector<uint32_t, mi_stl_allocator<uint32_t>>(), std::vector<uint32_t, mi_stl_allocator<uint32_t>>(),
                             std::vector<uint32_t, mi_stl_allocator<uint32_t>>());
    }

    void CopySubcol(const MultiValBin *full_bin, const std::vector<int, mi_stl_allocator<int>> &,
                    const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &lower,
                    const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &upper,
    ...
    void CopySubrowAndSubcol(const MultiValBin *full_bin,
                             ...
                             data_size_t num_used_indices,
                             const std::vector<int, mi_stl_allocator<int>> &,
                             const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &lower,
                             const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &upper,
                             const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &delta) override
    {
  ...
  private:
    ...
    std::vector<std::vector<VAL_T, Common::AlignmentAllocator<VAL_T, 32>>>
        t_data_;
    std::vector<INDEX_T, mi_stl_allocator<INDEX_T>> t_size_;
