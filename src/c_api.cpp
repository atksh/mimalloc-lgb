...
#define SHARED_LOCK(mtx) \
  ...
  public:
    ...
    {
      UNIQUE_LOCK(mutex_)
      std::vector<std::vector<int32_t, mi_stl_allocator<int32_t>>, mi_stl_allocator<std::vector<int32_t, mi_stl_allocator<int32_t>>>> v_leaf_preds(nrow, std::vector<int32_t, mi_stl_allocator<int32_t>>(ncol, 0));
    ...
    void PredictSingleRow(int predict_type, int ncol,
                          std::function<std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>>(int row_idx)> get_row_fun,
                          const Config &config,
...
#pragma omp parallel for schedule(static)
    ...
    void PredictSparse(int start_iteration, int num_iteration, int predict_type, int64_t nrow, int ncol,
                       std::function<std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>>(int64_t row_idx)> get_row_fun,
                       const Config &config, int64_t *out_elements_size,
                       std::vector<std::vector<std::unordered_map<int, double>, mi_stl_allocator<std::unordered_map<int, double>>>> *agg_ptr,
                       ...
                       bool *is_data_float32_ptr, int num_matrices) const
    {
      ...
      std::vector<std::vector<std::unordered_map<int, double>, mi_stl_allocator<std::unordered_map<int, double>>>> &agg = *agg_ptr;
      OMP_INIT_EX();
#pragma omp parallel for schedule(static)
      for (int64_t i = 0; i < nrow; ++i)
      {
        OMP_LOOP_EX_BEGIN();
        auto one_row = get_row_fun(i);
        agg[i] = std::vector<std::unordered_map<int, double>, mi_stl_allocator<std::unordered_map<int, double>>>(num_matrices);
                          ...
                          int32_t **out_indices, void **out_data, int data_type) const
    {
      ...
      std::vector<std::vector<std::unordered_map<int, double>, mi_stl_allocator<std::unordered_map<int, double>>>> agg(nrow);
      int64_t elements_size = 0;
      ...
      std::vector<int, mi_stl_allocator<int>> row_sizes(num_matrices * nrow);
      std::vector<int64_t, mi_stl_allocator<int64_t>> row_matrix_offsets(num_matrices * nrow);
...
#pragma omp parallel for schedule(static)
                          ...
                          int32_t **out_indices, void **out_data, int data_type) const
    {
      ...
      std::vector<std::vector<int64_t, mi_stl_allocator<int64_t>>> column_sizes(num_matrices);
      for (int m = 0; m < num_matrices; ++m)
      {
        column_sizes[m] = std::vector<int64_t, mi_stl_allocator<int64_t>>(num_output_cols, 0);
        for (int64_t i = 0; i < static_cast<int64_t>(agg.size()); ++i)
        {
          auto row_vector = agg[i];
          for (auto it = row_vector[m].begin(); it != row_vector[m].end(); ++it)
      ...
      std::vector<std::vector<int64_t, mi_stl_allocator<int64_t>>> column_counts(num_matrices);
      // keep track of beginning index for each column
      std::vector<std::vector<int64_t, mi_stl_allocator<int64_t>>> column_start_indices(num_matrices);
      // keep track of beginning index for each matrix
      std::vector<int64_t, mi_stl_allocator<int64_t>> matrix_start_indices(num_matrices, 0);
      int col_ptr_index = 0;
      for (int m = 0; m < num_matrices; ++m)
      {
        int64_t col_ptr_value = 0;
        column_start_indices[m] = std::vector<int64_t, mi_stl_allocator<int64_t>>(num_output_cols, 0);
        column_counts[m] = std::vector<int64_t, mi_stl_allocator<int64_t>>(num_output_cols, 0);
        if (is_col_ptr_int32)
        {
    ...
    std::vector<double, mi_stl_allocator<double>> FeatureImportance(int num_iteration, int importance_type) const
    {
  ...
  private:
    ...
    std::vector<std::unique_ptr<Metric>, mi_stl_allocator<std::unique_ptr<Metric>>> train_metric_;
    /*! \brief Metrics for validation data */
    std::vector<std::vector<std::unique_ptr<Metric>, mi_stl_allocator<std::unique_ptr<Metric>>>> valid_metrics_;
...
std::function<std::vector<double, mi_stl_allocator<double>>(int row_idx)>
RowFunctionFromDenseMatric(const void *data, int num_row, int num_col, int data_type, int is_row_major);

std::function<std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>>(int row_idx)>
...
template <typename T>
std::function<std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>>(T idx)>
                               ...
                               DatasetHandle *out)
{
  ...
  std::vector<std::function<std::vector<double, mi_stl_allocator<double>>(int row_idx)>> get_row_fun;
  for (int j = 0; j < nmat; ++j)
  {
  ...
  if (reference == nullptr)
  {
    ...
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> sample_values(ncol);
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> sample_idx(ncol);
                              ...
                              DatasetHandle *out)
{
  ...
  if (reference == nullptr)
  {
    ...
    sample_cnt = static_cast<int>(sample_indices.size());
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> sample_values(num_col);
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> sample_idx(num_col);
    for (size_t i = 0; i < sample_indices.size(); ++i)
    {
      auto idx = sample_indices[i];
                                  ...
                                  DatasetHandle *out)
{
  ...
  auto get_row_fun = *static_cast<std::function<void(int idx, std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &)> *>(get_row_funptr);
  auto param = Config::Str2Map(parameters);
  Config config;
  config.Set(param);
  if (config.num_threads > 0)
  ...
  if (reference == nullptr)
  {
    ...
    std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> buffer;
    for (size_t i = 0; i < sample_indices.size(); ++i)
    {
      auto idx = sample_indices[i];
  ...
  OMP_INIT_EX();
  std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> thread_buffer;
                              ...
                              DatasetHandle *out)
{
  ...
  if (reference == nullptr)
  {
    ...
    auto sample_indices = rand.Sample(nrow, sample_cnt);
    sample_cnt = static_cast<int>(sample_indices.size());
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> sample_values(ncol_ptr - 1);
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> sample_idx(ncol_ptr - 1);
    OMP_INIT_EX();
#pragma omp parallel for schedule(static)
    ...
    int num_feature_names)
{
  ...
  std::vector<std::string, mi_stl_allocator<std::string>> feature_names_str;
                                    ...
                                    void **out_data)
{
  ...
  else if (matrix_type == C_API_MATRIX_TYPE_CSC)
  {
    ...
    std::vector<std::vector<CSC_RowIterator, mi_stl_allocator<CSC_RowIterator>>> iterators(num_threads, std::vector<CSC_RowIterator, mi_stl_allocator<CSC_RowIterator>>());
    ...
    std::function<std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>>(int64_t row_idx)> get_row_fun =
        [&iterators, ncol](int64_t i) {
          std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> one_row;
                                  ...
                                  double *out_results)
{
  API_BEGIN();
  Booster *ref_booster = reinterpret_cast<Booster *>(handle);
  std::vector<double, mi_stl_allocator<double>> feature_importances = ref_booster->FeatureImportance(num_iteration, importance_type);
...
RowFunctionFromDenseMatric(const void *data, int num_row, int num_col, int data_type, int is_row_major)
{
  if (data_type == C_API_DTYPE_FLOAT32)
  {
    ...
    {
      return [=](int row_idx) {
        std::vector<double, mi_stl_allocator<double>> ret(num_col);
...
RowPairFunctionFromDenseMatric(const void *data, int num_row, int num_col, int data_type, int is_row_major)
{
  ...
  if (inner_function != nullptr)
  {
    return [inner_function](int row_idx) {
      ...
      std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> ret;
