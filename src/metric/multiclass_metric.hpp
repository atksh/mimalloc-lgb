...
*/
  ...
  public:
    ...
    const std::vector<std::string, mi_stl_allocator<std::string>> &GetName() const override
    {
      return name_;
    }
    ...
    std::vector<double, mi_stl_allocator<double>> Eval(const double *score, const ObjectiveFunction *objective) const override
    {
      double sum_loss = 0.0;
...
#pragma omp parallel for schedule(static) reduction(+ \
          ...
          for (data_size_t i = 0; i < num_data_; ++i)
          {
            std::vector<double, mi_stl_allocator<double>> rec(num_tree_per_iteration);
            for (int k = 0; k < num_tree_per_iteration; ++k)
            {
              size_t idx = static_cast<size_t>(num_data_) * k + i;
...
#pragma omp parallel for schedule(static) reduction(+ \
      ...
      return std::vector<double, mi_stl_allocator<double>>(1, loss);
    }
  ...
  private:
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> name_;
    int num_class_;
  ...
  public:
    ...
    inline static double LossOnPoint(label_t label, std::vector<double, mi_stl_allocator<double>> *score, const Config &config)
    {
      ...
      {
        if (ref_score[i] >= ref_score[k])
          ++num_larger;
        if (num_larger > config.multi_error_top_k)
          return 1.0f;
      }
      return 0.0f;
    }
  ...
  public:
    ...
    inline static double LossOnPoint(label_t label, std::vector<double, mi_stl_allocator<double>> *score, const Config &)
    {
      size_t k = static_cast<size_t>(label);
      auto &ref_score = *score;
  ...
  public:
    ...
    const std::vector<std::string, mi_stl_allocator<std::string>> &GetName() const override { return name_; }
    ...
    void Init(const Metadata &metadata, data_size_t num_data) override
    {
      ...
      sorted_data_idx_ = std::vector<data_size_t, mi_stl_allocator<data_size_t>>(num_data_, 0);
      for (data_size_t i = 0; i < num_data_; ++i)
      {
    ...
    std::vector<double, mi_stl_allocator<double>> Eval(const double *score, const ObjectiveFunction *) const override
    {
      ...
      auto class_sizes = std::vector<data_size_t, mi_stl_allocator<data_size_t>>(num_class_, 0);
      for (data_size_t i = 0; i < num_data_; ++i)
      {
      ...
      auto S = std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>>(num_class_, std::vector<double, mi_stl_allocator<double>>(num_class_, 0));
      int i_start = 0;
      for (int i = 0; i < num_class_; ++i)
      {
        ...
        {
          std::vector<double, mi_stl_allocator<double>> curr_v;
          ...
          std::vector<data_size_t, mi_stl_allocator<data_size_t>> class_i_j_indices;
          ...
          std::vector<std::pair<data_size_t, double>, mi_stl_allocator<std::pair<data_size_t, double>>> dist;
      ...
      return std::vector<double, mi_stl_allocator<double>>(1, ans);
    }
  ...
  private:
    ...
    int num_class_;
    /*! \brief class_weights*/
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> class_weights_;
    /*! \brief config parameters*/
    Config config_;
    /*! \brief index to data, sorted by true class*/
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> sorted_data_idx_;
  };
