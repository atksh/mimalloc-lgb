...
namespace LightGBM
{
  ...
  std::vector<double, mi_stl_allocator<double>> DCGCalculator::label_gain_;
  std::vector<double, mi_stl_allocator<double>> DCGCalculator::discount_;
  const data_size_t DCGCalculator::kMaxPosition = 10000;

  void DCGCalculator::DefaultEvalAt(std::vector<int, mi_stl_allocator<int>> *eval_at)
  {
  ...
  void DCGCalculator::DefaultLabelGain(std::vector<double, mi_stl_allocator<double>> *label_gain)
  {
    if (!label_gain->empty())
    {
      return;
  ...
  void DCGCalculator::Init(const std::vector<double, mi_stl_allocator<double>> &input_label_gain)
  {
  ...
  double DCGCalculator::CalMaxDCGAtK(data_size_t k, const label_t *label, data_size_t num_data)
  {
    ...
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> label_cnt(label_gain_.size(), 0);
  ...
  void DCGCalculator::CalMaxDCG(const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &ks,
                                const label_t *label,
                                data_size_t num_data,
                                std::vector<double, mi_stl_allocator<double>> *out)
  {
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> label_cnt(label_gain_.size(), 0);
    // counts for all labels
    for (data_size_t i = 0; i < num_data; ++i)
                                  ...
                                  const double *score, data_size_t num_data)
  {
    ...
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> sorted_idx(num_data);
    for (data_size_t i = 0; i < num_data; ++i)
    {
      sorted_idx[i] = i;
    }
  ...
  void DCGCalculator::CalDCG(const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &ks, const label_t *label,
                             const double *score, data_size_t num_data, std::vector<double, mi_stl_allocator<double>> *out)
  {
    // get sorted indices by score
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> sorted_idx(num_data);
    for (data_size_t i = 0; i < num_data; ++i)
