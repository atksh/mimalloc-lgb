...
#pragma omp parallel for schedule(static)
    ...
    std::vector<double, mi_stl_allocator<double>> Eval(const double *score, const ObjectiveFunction *) const override
    {
      ...
      std::vector<double, mi_stl_allocator<double>> tmp_dcg(eval_at_.size(), 0.0f);
      if (query_weights_ == nullptr)
      {
...
#pragma omp parallel for schedule(static) firstprivate(tmp_dcg)
  ...
  private:
    ...
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> inverse_max_dcgs_;
  };
