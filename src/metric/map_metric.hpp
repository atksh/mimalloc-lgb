...
namespace LightGBM
{
  ...
  public:
    ...
    void CalMapAtK(std::vector<int, mi_stl_allocator<int>> ks, data_size_t npos, const label_t *label,
                   const double *score, data_size_t num_data, std::vector<double, mi_stl_allocator<double>> *out) const
    {
      // get sorted indices by score
      std::vector<data_size_t, mi_stl_allocator<data_size_t>> sorted_idx;
    ...
    std::vector<double, mi_stl_allocator<double>> Eval(const double *score, const ObjectiveFunction *) const override
    {
      ...
      std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> result_buffer_;
      for (int i = 0; i < num_threads; ++i)
      {
        result_buffer_.emplace_back(eval_at_.size(), 0.0f);
      }
      std::vector<double, mi_stl_allocator<double>> tmp_map(eval_at_.size(), 0.0f);
      if (query_weights_ == nullptr)
      {
...
#pragma omp parallel for schedule(guided) firstprivate(tmp_map)
      ...
      std::vector<double, mi_stl_allocator<double>> result(eval_at_.size(), 0.0f);
  ...
  private:
    ...
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> eval_at_;
    std::vector<std::string, mi_stl_allocator<std::string>> name_;
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> npos_per_query_;
  };
