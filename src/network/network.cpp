...
namespace LightGBM
{
  ...
  THREAD_LOCAL std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> Network::block_start_;
  THREAD_LOCAL std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> Network::block_len_;
  THREAD_LOCAL comm_size_t Network::buffer_size_ = 0;
  THREAD_LOCAL std::vector<char, mi_stl_allocator<char>> Network::buffer_;
  ...
  void Network::Init(Config config)
  {
    if (config.num_machines > 1)
    {
      ...
      block_start_ = std::vector<comm_size_t, mi_stl_allocator<comm_size_t>>(num_machines_);
      block_len_ = std::vector<comm_size_t, mi_stl_allocator<comm_size_t>>(num_machines_);
