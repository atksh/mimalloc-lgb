...
namespace LightGBM
{
  ...
  BruckMap BruckMap::Construct(int rank, int num_machines)
  {
    ...
    std::vector<int, mi_stl_allocator<int>> distance;
    int k = 0;
  ...
  RecursiveHalvingMap RecursiveHalvingMap::Construct(int rank, int num_machines)
  {
    ...
    else
    {
      ...
      std::vector<RecursiveHalvingNodeType, mi_stl_allocator<RecursiveHalvingNodeType>> node_type(num_machines);
      ...
      std::vector<int, mi_stl_allocator<int>> group_block_start(lower_power_of_2);
      std::vector<int, mi_stl_allocator<int>> group_block_len(lower_power_of_2, 0);
      // convert from group to node leader
      std::vector<int, mi_stl_allocator<int>> group_to_node(lower_power_of_2);
      // convert from node to group
      std::vector<int, mi_stl_allocator<int>> node_to_group(num_machines);
