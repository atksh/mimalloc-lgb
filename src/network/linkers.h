...
*/
  ...
  */
  ...
  private:
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> client_ips_;
    /*! \brief use to store client ports */
    std::vector<int, mi_stl_allocator<int>> client_ports_;
    /*! \brief time out for sockets, in minutes */
    int socket_timeout_;
    /*! \brief Local listen ports */
    int local_listen_port_;
    /*! \brief Linkers */
    std::vector<std::unique_ptr<TcpSocket>, mi_stl_allocator<std::unique_ptr<TcpSocket>>> linkers_;
