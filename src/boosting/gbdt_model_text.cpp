...
namespace LightGBM
{
  ...
  std::string GBDT::DumpModel(int start_iteration, int num_iteration, int feature_importance_type) const
  {
    ...
    std::vector<double, mi_stl_allocator<double>> feature_importances = FeatureImportance(
        num_iteration, feature_importance_type);
    // store the importance first
    std::vector<std::pair<size_t, std::string>, mi_stl_allocator<std::pair<size_t, std::string>>> pairs;
  ...
  std::string GBDT::SaveModelToString(int start_iteration, int num_iteration, int feature_importance_type) const
  {
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> tree_strs(num_used_model - start_model);
    std::vector<size_t, mi_stl_allocator<size_t>> tree_sizes(num_used_model - start_model);
// output tree models
#pragma omp parallel for schedule(static)
  ...
  bool GBDT::LoadModelFromString(const char *buffer, size_t len)
  {
    ...
    {
      std::vector<size_t, mi_stl_allocator<size_t>> tree_sizes = Common::StringToArray<size_t>(key_vals["tree_sizes"].c_str(), ' ');
      std::vector<size_t, mi_stl_allocator<size_t>> tree_boundries(tree_sizes.size() + 1, 0);
...
#pragma omp parallel for schedule(static)
  ...
  std::vector<double, mi_stl_allocator<double>> GBDT::FeatureImportance(int num_iteration, int importance_type) const
  {
    int num_used_model = static_cast<int>(models_.size());
    if (num_iteration > 0)
    {
    ...
    std::vector<double, mi_stl_allocator<double>> feature_importances(max_feature_idx_ + 1, 0.0);
