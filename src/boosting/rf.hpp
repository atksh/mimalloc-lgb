...
*/
  ...
  public:
    ...
    void Boosting() override
    {
      ...
      std::vector<double, mi_stl_allocator<double>> tmp_scores(total_size, 0.0f);
#pragma omp parallel for schedule(static)
      ...
      objective_function_->GetGradients(tmp_scores.data(), gradients_.data(), hessians_.data());
    }
  ...
  private:
    std::vector<score_t, mi_stl_allocator<score_t>> tmp_grad_;
    std::vector<score_t, mi_stl_allocator<score_t>> tmp_hess_;
    std::vector<double, mi_stl_allocator<double>> init_scores_;
  };
