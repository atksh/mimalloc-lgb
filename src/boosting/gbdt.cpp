...
namespace LightGBM
{
  ...
  void GBDT::Init(const Config *config, const Dataset *train_data, const ObjectiveFunction *objective_function,
                  const std::vector<const Metric *, mi_stl_allocator<const Metric *>> &training_metrics)
  {
    ...
    class_need_train_ = std::vector<bool, mi_stl_allocator<bool>>(num_tree_per_iteration_, true);
  ...
  void GBDT::AddValidDataset(const Dataset *valid_data,
                             const std::vector<const Metric *, mi_stl_allocator<const Metric *>> &valid_metrics)
  {
  ...
  void GBDT::RefitTree(const std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> &tree_leaf_prediction)
  {
    ...
    std::vector<int, mi_stl_allocator<int>> leaf_pred(num_data_);
...
*/
  ...
  bool GBDT::TrainOneIter(const score_t *gradients, const score_t *hessians)
  {
    ...
    std::vector<double, mi_stl_allocator<double>> init_scores(num_tree_per_iteration_, 0.0);
    // boosting first
  ...
  std::vector<double, mi_stl_allocator<double>> GBDT::EvalOneMetric(const Metric *metric, const double *score) const
  {
  ...
  std::vector<double, mi_stl_allocator<double>> GBDT::GetEvalAt(int data_idx) const
  {
    CHECK(data_idx >= 0 && data_idx <= static_cast<int>(valid_score_updater_.size()));
    std::vector<double, mi_stl_allocator<double>> ret;
    if (data_idx == 0)
    {
      for (auto &sub_metric : training_metrics_)
      {
  ...
  void GBDT::PredictContribByMap(const std::unordered_map<int, double> &features,
                                 std::vector<std::unordered_map<int, double>, mi_stl_allocator<std::unordered_map<int, double>>> *output) const
  {
    const int num_features = max_feature_idx_ + 1;
    const int end_iteration_for_pred = start_iteration_for_pred_ + num_iteration_for_pred_;
    for (int i = start_iteration_for_pred_; i < end_iteration_for_pred; ++i)
...
#pragma omp parallel for schedule(static)
      ...
      {
        std::vector<double, mi_stl_allocator<double>> tree_pred(num_tree_per_iteration_);
        for (int j = 0; j < num_tree_per_iteration_; ++j)
        {
          tree_pred[j] = raw_scores[j * num_data + i];
        }
        std::vector<double, mi_stl_allocator<double>> tmp_result(num_class_);
