...
*/
  ...
  public:
    ...
    explicit MulticlassSoftmax(const std::vector<std::string, mi_stl_allocator<std::string>> &strs)
    {
      num_class_ = -1;
      for (auto str : strs)
      {
...
#pragma omp parallel for schedule(static) private(rec)
        for (data_size_t i = 0; i < num_data_; ++i)
        {
          ...
          for (int k = 0; k < num_class_; ++k)
          {
            ...
            size_t idx = static_cast<size_t>(num_data_) * k + i;
            double mask = static_cast<double>(noise[i] < static_cast<float>(w[label_int_[i]]));
            if (label_int_[i] == k)
            {
...
#pragma omp parallel for schedule(static) private(rec)
    ...
    {
      if (std::fabs(class_init_probs_[class_id]) <= kEpsilon || std::fabs(class_init_probs_[class_id]) >= 1.0 - kEpsilon)
      {
        return false;
      }
      else
  ...
  private:
    ...
    std::vector<int, mi_stl_allocator<int>> label_int_;
...
*/
  ...
  public:
    ...
    explicit MulticlassOVA(const std::vector<std::string, mi_stl_allocator<std::string>> &strs)
    {
      num_class_ = -1;
      sigmoid_ = -1;
      for (auto str : strs)
  ...
  private:
    ...
    /*! \brief Number of classes */
    int num_class_;
    std::vector<std::unique_ptr<BinaryLogloss>, mi_stl_allocator<std::unique_ptr<BinaryLogloss>>> binary_loss_;
    double sigmoid_;
  };
