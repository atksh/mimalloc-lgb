...
#define PercentileFun(T, data_reader, cnt_data, alpha)                    \
  {                                                                       \
    ...
    std::vector<T, mi_stl_allocator<T>> ref_data(cnt_data);               \
...
#define WeightedPercentileFun(T, data_reader, weight_reader, cnt_data, alpha)     \
  {                                                                               \
    ...
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> sorted_idx(cnt_data); \
    for (data_size_t i = 0; i < cnt_data; ++i)                                    \
    {                                                                             \
      sorted_idx[i] = i;                                                          \
    }                                                                             \
    ...
    std::vector<double, mi_stl_allocator<double>> weighted_cdf(cnt_data);         \
...
*/
  ...
  public:
    ...
    explicit RegressionL2loss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs)
    {
      sqrt_ = false;
      for (auto str : strs)
      {
...
#pragma omp parallel for schedule(static) reduction(+ \
  ...
  protected:
    ...
    std::vector<label_t, mi_stl_allocator<label_t>> trans_label_;
  };
...
*/
  ...
  public:
    ...
    explicit RegressionL1loss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionL2loss(strs)
    {
    }
...
*/
  ...
  public:
    ...
    explicit RegressionHuberLoss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionL2loss(strs)
    {
      if (sqrt_)
      {
        Log::Warning("Cannot use sqrt transform in %s Regression, will auto disable it", GetName());
...
#pragma omp parallel for schedule(static)
  ...
  public:
    ...
    explicit RegressionFairLoss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionL2loss(strs)
    {
    }
...
*/
  ...
  public:
    ...
    explicit RegressionPoissonLoss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionL2loss(strs)
    {
    }
...
#pragma omp parallel for schedule(static)
  ...
  public:
    ...
    explicit RegressionQuantileloss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionL2loss(strs)
    {
    }
...
*/
  ...
  public:
    ...
    explicit RegressionMAPELOSS(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionL1loss(strs)
    {
    }
...
#undef weight_reader
  ...
  private:
    std::vector<label_t, mi_stl_allocator<label_t>> label_weight_;
  };
...
*/
  ...
  public:
    ...
    explicit RegressionGammaLoss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionPoissonLoss(strs)
    {
    }
...
*/
  ...
  public:
    ...
    explicit RegressionTweedieLoss(const std::vector<std::string, mi_stl_allocator<std::string>> &strs) : RegressionPoissonLoss(strs)
    {
    }
