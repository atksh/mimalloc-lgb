 ...
 */
  ...
  public:
    ...
    explicit RankingObjective(const std::vector<std::string, mi_stl_allocator<std::string>> &) : seed_(0) {}
 ...
 */
  ...
  public:
    ...
    explicit LambdarankNDCG(const std::vector<std::string, mi_stl_allocator<std::string>> &strs)
...
#pragma omp parallel for schedule(static)
                                        ...
                                        score_t *hessians) const override
    {
      ...
      std::vector<data_size_t, mi_stl_allocator<data_size_t>> sorted_idx(cnt);
      for (data_size_t i = 0; i < cnt; ++i)
      {
        sorted_idx[i] = i;
      }
  ...
  private:
    ...
    std::vector<double, mi_stl_allocator<double>> inverse_max_dcgs_;
    /*! \brief Cache result for sigmoid transform to speed up */
    std::vector<double, mi_stl_allocator<double>> sigmoid_table_;
    /*! \brief Gains for labels */
    std::vector<double, mi_stl_allocator<double>> label_gain_;
 ...
 */
  ...
  public:
    ...
    explicit RankXENDCG(const std::vector<std::string, mi_stl_allocator<std::string>> &strs)
        : RankingObjective(strs) {}
                                        ...
                                        score_t *hessians) const override
    {
      ...
      std::vector<double, mi_stl_allocator<double>> rho(cnt, 0.0);
      ...
      std::vector<double, mi_stl_allocator<double>> params(cnt);
  ...
  private:
    mutable std::vector<Random, mi_stl_allocator<Random>> rands_;
  };
