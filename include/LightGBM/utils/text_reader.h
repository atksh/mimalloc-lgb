...
*/
  ...
  */
    inline std::vector<std::string, mi_stl_allocator<std::string>> &Lines() { return lines_; }
  ...
  */
    ...
    std::vector<char, mi_stl_allocator<char>> ReadContent(size_t *out_len)
    {
      std::vector<char, mi_stl_allocator<char>> ret;
      *out_len = 0;
      ...
      auto buffer_read = std::vector<char, mi_stl_allocator<char>>(buffer_size);
      size_t read_cnt = 0;
      do
      {
    ...
    INDEX_T SampleFromFile(Random *random, INDEX_T sample_cnt, std::vector<std::string, mi_stl_allocator<std::string>> *out_sampled_data)
    {
  ...
  */
    INDEX_T ReadAndFilterLines(const std::function<bool(INDEX_T)> &filter_fun, std::vector<INDEX_T, mi_stl_allocator<INDEX_T>> *out_used_data_indices)
    {
    ...
    INDEX_T SampleAndFilterFromFile(const std::function<bool(INDEX_T)> &filter_fun, std::vector<INDEX_T, mi_stl_allocator<INDEX_T>> *out_used_data_indices,
                                    Random *random, INDEX_T sample_cnt, std::vector<std::string, mi_stl_allocator<std::string>> *out_sampled_data)
    {
      INDEX_T cur_sample_cnt = 0;
      out_used_data_indices->clear();
      INDEX_T total_cnt = ReadAllAndProcess(
    ...
    INDEX_T ReadAllAndProcessParallelWithFilter(const std::function<void(INDEX_T, const std::vector<std::string, mi_stl_allocator<std::string>> &)> &process_fun, const std::function<bool(INDEX_T, INDEX_T)> &filter_fun)
    {
      last_line_ = "";
      INDEX_T total_cnt = 0;
      size_t bytes_read = 0;
    ...
    INDEX_T ReadAllAndProcessParallel(const std::function<void(INDEX_T, const std::vector<std::string, mi_stl_allocator<std::string>> &)> &process_fun)
    {
      return ReadAllAndProcessParallelWithFilter(process_fun, [](INDEX_T, INDEX_T) { return true; });
    }

    INDEX_T ReadPartAndProcessParallel(const std::vector<INDEX_T, mi_stl_allocator<INDEX_T>> &used_data_indices, const std::function<void(INDEX_T, const std::vector<std::string, mi_stl_allocator<std::string>> &)> &process_fun)
    {
  ...
  private:
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> lines_;
