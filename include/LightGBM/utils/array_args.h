...
*/
  ...
  {
  public:
    inline static size_t ArgMaxMT(const std::vector<VAL_T, mi_stl_allocator<VAL_T>> &array)
    {
      int num_threads = OMP_NUM_THREADS();
      std::vector<size_t, mi_stl_allocator<size_t>> arg_maxs(num_threads, 0);
    ...
    inline static size_t ArgMax(const std::vector<VAL_T, mi_stl_allocator<VAL_T>> &array)
    {
      if (array.empty())
      {
        return 0;
    ...
    inline static size_t ArgMin(const std::vector<VAL_T, mi_stl_allocator<VAL_T>> &array)
    {
      if (array.empty())
      {
        return 0;
    ...
    inline static void Partition(std::vector<VAL_T, mi_stl_allocator<VAL_T>> *arr, int start, int end, int *l, int *r)
    {
      int i = start - 1;
      int j = end - 1;
      int p = i;
      ...
      std::vector<VAL_T, mi_stl_allocator<VAL_T>> &ref = *arr;
      VAL_T v = ref[end - 1];
      for (;;)
      {
        while (ref[++i] > v)
    ...
    inline static int ArgMaxAtK(std::vector<VAL_T, mi_stl_allocator<VAL_T>> *arr, int start, int end, int k)
    {
      if (start >= end - 1)
      {
        return start;
    ...
    inline static void MaxK(const std::vector<VAL_T, mi_stl_allocator<VAL_T>> &array, int k, std::vector<VAL_T, mi_stl_allocator<VAL_T>> *out)
    {
      out->clear();
      if (k <= 0)
      {
    ...
    inline static void Assign(std::vector<VAL_T, mi_stl_allocator<VAL_T>> *array, VAL_T t, size_t n)
    {
      array->resize(n);
    ...
    inline static bool CheckAllZero(const std::vector<VAL_T, mi_stl_allocator<VAL_T>> &array)
    {
      for (size_t i = 0; i < array.size(); ++i)
      {
        if (array[i] != VAL_T(0))
    ...
    inline static bool CheckAll(const std::vector<VAL_T, mi_stl_allocator<VAL_T>> &array, VAL_T t)
    {
      for (size_t i = 0; i < array.size(); ++i)
      {
        if (array[i] != t)
