...
#pragma region Core Parameters
    ...
    // desc = **Note**: can be used only in CLI version
    std::vector<std::string, mi_stl_allocator<std::string>> valid;
...
#pragma region Learning Control Parameters
    ...
    std::vector<int8_t, mi_stl_allocator<int8_t>> monotone_constraints;
    ...
    std::vector<double, mi_stl_allocator<double>> feature_contri;
    ...
    std::vector<double, mi_stl_allocator<double>> cegb_penalty_feature_lazy;

    // type = multi-double
    // default = 0,0,...,0
    // desc = cost-effective gradient boosting penalty for using a feature
    // desc = applied once per forest
    std::vector<double, mi_stl_allocator<double>> cegb_penalty_feature_coupled;
...
#pragma region Dataset Parameters
    ...
    std::vector<int32_t, mi_stl_allocator<int32_t>> max_bin_by_feature;
...
#pragma region Objective Parameters
    ...
    // desc = separate by ``,``
    std::vector<double, mi_stl_allocator<double>> label_gain;
...
#pragma region Metric Parameters
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> metric;
    ...
    std::vector<int, mi_stl_allocator<int>> eval_at;
    ...
    std::vector<double, mi_stl_allocator<double>> auc_mu_weights;
...
#pragma endregion
    ...
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> auc_mu_weights_matrix;
    std::vector<std::vector<int, mi_stl_allocator<int>>, mi_stl_allocator<std::vector<int, mi_stl_allocator<int>>>> interaction_constraints_vector;
  ...
  inline std::string ParseObjectiveAlias(const std::string &type)
  {
    ...
    else if (type == std::string("rank_xendcg") || type == std::string("xendcg") || type == std::string("xe_ndcg") || type == std::string("xe_ndcg_mart") || type == std::string("xendcg_mart"))
    {
      return "rank_xendcg";
    }
  ...
  inline std::string ParseMetricAlias(const std::string &type)
  {
    ...
    else if (type == std::string("ndcg") || type == std::string("lambdarank") || type == std::string("rank_xendcg") || type == std::string("xendcg") || type == std::string("xe_ndcg") || type == std::string("xe_ndcg_mart") || type == std::string("xendcg_mart"))
    {
      return "ndcg";
    }
