...
*          and store some meta information for bin*/
  ...
  */
    void FindBin(double *values, int num_values, size_t total_sample_cnt, int max_bin, int min_data_in_bin, int min_split_data, bool pre_filter, BinType bin_type,
                 bool use_missing, bool zero_as_missing, const std::vector<double, mi_stl_allocator<double>> &forced_upper_bounds);
  ...
  */
  ...
  private:
    ...
    std::vector<double, mi_stl_allocator<double>> bin_upper_bound_;
    ...
    std::vector<int, mi_stl_allocator<int>> bin_2_categorical_;
...
*/
  ...
  */
    virtual void LoadFromMemory(const void *memory,
                                const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &local_used_indices) = 0;
  ...
  */
  ...
  public:
    ...
    virtual void PushOneRow(int tid, data_size_t idx, const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &values) = 0;
    ...
    virtual void CopySubcol(const MultiValBin *full_bin,
                            ...
                            const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &lower,
                            const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &upper,
                            const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &delta) = 0;
    ...
    virtual void CopySubrowAndSubcol(
        ...
        data_size_t num_used_indices, const std::vector<int, mi_stl_allocator<int>> &used_feature_index,
        const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &lower, const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &upper,
        const std::vector<uint32_t, mi_stl_allocator<uint32_t>> &delta) = 0;
