...
namespace LightGBM
{
  ...
  public:
    ...
    std::vector<int, mi_stl_allocator<int>> in_ranks;
    /*! \brief out_ranks[i] means the out rank on i-th communication */
    std::vector<int, mi_stl_allocator<int>> out_ranks;
    BruckMap();
...
*/
  ...
  */
    ...
    template <class T>
    static std::vector<T, mi_stl_allocator<T>> GlobalSum(std::vector<T, mi_stl_allocator<T>> *local)
    {
      std::vector<T, mi_stl_allocator<T>> global(local->size(), 0);
    ...
    template <class T>
    static std::vector<T, mi_stl_allocator<T>> GlobalArray(T local)
    {
      std::vector<T, mi_stl_allocator<T>> global(num_machines_, 0);
      int type_size = sizeof(T);
      std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_start(num_machines_);
      std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_len(num_machines_, type_size);
  ...
  private:
    ...
    static THREAD_LOCAL std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_start_;
    /*! \brief Buffer to store block size */
    static THREAD_LOCAL std::vector<comm_size_t, mi_stl_allocator<comm_size_t>> block_len_;
    /*! \brief Buffer  */
    static THREAD_LOCAL std::vector<char, mi_stl_allocator<char>> buffer_;
