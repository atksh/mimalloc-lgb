...
*/
  ...
  */
    void PartitionLabel(const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &used_indices);
  ...
  */
    void CheckOrPartition(data_size_t num_all_data,
                          const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &used_data_indices);
  ...
  */
  ...
  private:
    ...
    /*! \brief Label data */
    std::vector<label_t, mi_stl_allocator<label_t>> label_;
    /*! \brief Weights data */
    std::vector<label_t, mi_stl_allocator<label_t>> weights_;
    /*! \brief Query boundaries */
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> query_boundaries_;
    /*! \brief Query weights */
    std::vector<label_t, mi_stl_allocator<label_t>> query_weights_;
    /*! \brief Number of querys */
    data_size_t num_queries_;
    /*! \brief Number of Initial score, used to check correct weight file */
    int64_t num_init_score_;
    /*! \brief Initial score */
    std::vector<double, mi_stl_allocator<double>> init_score_;
    /*! \brief Queries data */
    std::vector<data_size_t, mi_stl_allocator<data_size_t>> queries_;
  ...
  */
    virtual void ParseOneLine(const char *str,
                              std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> *out_features, double *out_label) const = 0;
  ...
  */
  ...
  struct TrainingShareStates
  {
    ...
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> hist_move_src;
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> hist_move_dest;
    std::vector<uint32_t, mi_stl_allocator<uint32_t>> hist_move_size;
...
*/
  ...
  public:
    ...
    inline void FinishOneRow(int tid, data_size_t row_idx, const std::vector<bool, mi_stl_allocator<bool>> &is_feature_added)
    {
      if (is_finish_load_)
      {
        return;
    ...
    inline void PushOneRow(int tid, data_size_t row_idx, const std::vector<double, mi_stl_allocator<double>> &feature_values)
    {
      if (is_finish_load_)
      {
        return;
    ...
    inline void PushOneRow(int tid, data_size_t row_idx, const std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &feature_values)
    {
      if (is_finish_load_)
      {
        return;
      }
      std::vector<bool, mi_stl_allocator<bool>> is_feature_added(num_features_, false);
    ...
    inline std::vector<int, mi_stl_allocator<int>> ValidFeatureIndices() const
    {
  ...
  */
    ...
    void InitTrain(const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used,
                   TrainingShareStates *share_state) const;

    template <bool USE_INDICES, bool USE_HESSIAN>
    void ConstructHistogramsInner(const std::vector<int8_t, mi_stl_allocator<int8_t>> &is_feature_used,
  ...
  */
    ...
    inline const std::vector<std::string, mi_stl_allocator<std::string>> &feature_names() const { return feature_names_; }

    inline void set_feature_names(const std::vector<std::string, mi_stl_allocator<std::string>> &feature_names)
    {
      ...
      feature_names_ = std::vector<std::string, mi_stl_allocator<std::string>>(feature_names);
    ...
    inline std::vector<std::string, mi_stl_allocator<std::string>> feature_infos() const
    {
      std::vector<std::string, mi_stl_allocator<std::string>> bufs;
      for (int i = 0; i < num_total_features_; ++i)
      {
  ...
  private:
    ...
    std::vector<std::unique_ptr<FeatureGroup>, mi_stl_allocator<std::unique_ptr<FeatureGroup>>> feature_groups_;
    /*! \brief Mapper from real feature index to used index*/
    std::vector<int, mi_stl_allocator<int>> used_feature_map_;
    ...
    std::vector<std::string, mi_stl_allocator<std::string>> feature_names_;
    /*! \brief store feature names */
    static const char *binary_file_token;
    int num_groups_;
    std::vector<int, mi_stl_allocator<int>> real_feature_idx_;
    std::vector<int, mi_stl_allocator<int>> feature2group_;
    std::vector<int, mi_stl_allocator<int>> feature2subfeature_;
    std::vector<uint64_t, mi_stl_allocator<uint64_t>> group_bin_boundaries_;
    std::vector<int, mi_stl_allocator<int>> group_feature_start_;
    std::vector<int, mi_stl_allocator<int>> group_feature_cnt_;
    bool is_finish_load_;
    int max_bin_;
    std::vector<int32_t, mi_stl_allocator<int32_t>> max_bin_by_feature_;
    std::vector<std::vector<double, mi_stl_allocator<double>>, mi_stl_allocator<std::vector<double, mi_stl_allocator<double>>>> forced_bin_bounds_;
    int bin_construct_sample_cnt_;
    int min_data_in_bin_;
    bool use_missing_;
    bool zero_as_missing_;
    std::vector<int, mi_stl_allocator<int>> feature_need_push_zeros_;
  };
