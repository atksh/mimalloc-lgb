...
*/
  ...
  */
    ...
    virtual const std::vector<std::string, mi_stl_allocator<std::string>> &GetName() const = 0;
  ...
  */
    virtual std::vector<double, mi_stl_allocator<double>> Eval(const double *score, const ObjectiveFunction *objective) const = 0;
...
*/
  ...
  {
  public:
    static void DefaultEvalAt(std::vector<int, mi_stl_allocator<int>> *eval_at);
    static void DefaultLabelGain(std::vector<double, mi_stl_allocator<double>> *label_gain);
    /*!
  * \brief Initial logic
  * \param label_gain Gain for labels, default is 2^i - 1
  */
    static void Init(const std::vector<double, mi_stl_allocator<double>> &label_gain);
  ...
  */
    static void CalDCG(const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &ks,
                       const label_t *label, const double *score,
                       data_size_t num_data, std::vector<double, mi_stl_allocator<double>> *out);
  ...
  * \param out Output result
  */
    static void CalMaxDCG(const std::vector<data_size_t, mi_stl_allocator<data_size_t>> &ks,
                          const label_t *label, data_size_t num_data, std::vector<double, mi_stl_allocator<double>> *out);
  ...
  */
  ...
  private:
    ...
    static std::vector<double, mi_stl_allocator<double>> label_gain_;
    /*! \brief store discount score for different position */
    static std::vector<double, mi_stl_allocator<double>> discount_;
