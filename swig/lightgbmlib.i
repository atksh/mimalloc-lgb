 ...
 */
/* lightgbmlib.i */
% module lightgbmlib % ignore LGBM_BoosterSaveModelToString;
...
#include "../include/LightGBM/c_api.h"
    ...
    % include "various.i" % include "carrays.i" % include "cpointer.i" % include "stdint.i"
...
*/
    ...
    /* header files */
    % include "../include/LightGBM/export.h" % include "../include/LightGBM/c_api.h"
...
#include <vector>
                                     ...
                                     DatasetHandle *out)
  {
    ...
    std::vector<CSRDirect, mi_stl_allocator<CSRDirect>> jniCache;
    ...
    std::function<void(int idx, std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &ret)> row_func = [&](int row_num, std::vector<std::pair<int, double>, mi_stl_allocator<std::pair<int, double>>> &ret) {
...
% pointer_functions(int, intp) % pointer_functions(long, longp) % pointer_functions(double, doublep) % pointer_functions(float, floatp) % pointer_functions(int64_t, int64_tp) % pointer_functions(int32_t, int32_tp)

    % pointer_cast(int64_t *, long *, int64_t_to_long_ptr) % pointer_cast(int64_t *, double *, int64_t_to_double_ptr) % pointer_cast(int32_t *, int *, int32_t_to_int_ptr) % pointer_cast(long *, int64_t *, long_to_int64_t_ptr) % pointer_cast(double *, int64_t *, double_to_int64_t_ptr) % pointer_cast(int *, int32_t *, int_to_int32_t_ptr)

    % pointer_cast(double *, void *, double_to_voidp_ptr) % pointer_cast(float *, void *, float_to_voidp_ptr) % pointer_cast(int *, void *, int_to_voidp_ptr) % pointer_cast(int32_t *, void *, int32_t_to_voidp_ptr) % pointer_cast(int64_t *, void *, int64_t_to_voidp_ptr)

    /* Custom pointer manipulation template */
    % define % pointer_manipulation(TYPE, NAME) % {static TYPE * new_##NAME(){ % } % {TYPE *NAME = new TYPE;
return NAME;
%
}
% {}
...
% enddef
    ...
    % define % pointer_dereference(TYPE, NAME) % {static TYPE NAME##_value(TYPE * self){TYPE NAME = *self;
return NAME;
}
%
}
...
% enddef
    ...
    % define % pointer_handle(TYPE, NAME) % {static TYPE * NAME##_handle(){ % } % {TYPE *NAME = new TYPE;
*NAME = (TYPE) operator new(sizeof(int *));
return NAME;
%
}
% {} %
...
% enddef
    ...
    % define % long_array_functions(TYPE, NAME) % {static TYPE * new_##NAME(int64_t nelements){ % } % {return new TYPE[nelements]();
%
}
% {}
...
% enddef
    ...
    % long_array_functions(double, doubleArray) % long_array_functions(float, floatArray) % long_array_functions(int, intArray) % long_array_functions(long, longArray)
